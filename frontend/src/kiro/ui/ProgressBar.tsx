...
export interface ProgressBarProps {
  ...
  color: string;
  height?: number; // 6-9
  /** role=progressbar erişilebilir adı — a11y için sağla (ör. "Matematik ilerlemesi") */
  ariaLabel?: string;
}

export function ProgressBar({ pct, color: barColor, height = 8, ariaLabel }: ProgressBarProps) {
  const theme = useKiroTheme();
  const s = surf(theme);
  const clamped = Math.max(0, Math.min(100, pct));
  return (
    <div style={{ height, backgroundColor: s.skeleton, borderRadius: radius.pill, overflow: 'hidden' }}
      role="progressbar" aria-label={ariaLabel} aria-valuenow={clamped} aria-valuemin={0} aria-valuemax={100}>
