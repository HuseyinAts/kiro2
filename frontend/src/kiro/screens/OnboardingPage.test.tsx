...
describe('OnboardingPage', () => {
  ...
  it('ton seçince yanıt görünür + CTA etkinleşir', async () => {
    render(<OnboardingPage />);
    expect(screen.getByRole('button', { name: /Seviyeni ölçelim/ })).toBeDisabled();
    ...
    expect(screen.getByRole('button', { name: /Seviyeni ölçelim/ })).toBeEnabled();
  });

  it('tam akış: ton → 6 soru yerleştirme → Planın hazır', async () => {
    render(<OnboardingPage />);
    await userEvent.click(screen.getByRole('radio', { name: 'Genelde sakinim' }));
    await userEvent.click(screen.getByRole('button', { name: /Seviyeni ölçelim/ }));
