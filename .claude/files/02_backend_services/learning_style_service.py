...
class LearningStyleService:
    """
    VARK + Felder-Silverman = 64 farklı öğrenme profili
    Dünyada ilk kez bu kadar detaylı profilleme!
    """
    ...
    async def detect_learning_style(
        ...
        hibrit_profil = {
            ...
            "profil_aciklamasi": f"64 kombinasyondan biri: {hibrit_kod}"
        }
    ...
    async def get_learning_recommendations(
        self, student_id: str, subject: str = "genel"
    ) -> List[Dict[str, Any]]:
        """Öğrenme stiline göre kişiselleştirilmiş öneriler"""
    ...
    def _generate_hibrit_code(
        self, vark_profile: Dict[str, float],
        felder_profile: Dict[str, float]
    ) -> str:
        """64 kombinasyondan birini oluştur"""
        
        # VARK kodu
        vark_code = max(vark_profile, key=vark_profile.get)[0].upper()
        
        # Felder-Silverman kodu
        felder_code = ""
        
        # Active/Reflective
        felder_code += "A" if felder_profile["active_reflective"] > 0 else "R"
        
        # Sensing/Intuitive
        felder_code += "S" if felder_profile["sensing_intuitive"] > 0 else "I"
        
        # Visual/Verbal
        felder_code += "V" if felder_profile["visual_verbal"] > 0 else "B"
        
        # Sequential/Global
        felder_code += "S" if felder_profile["sequential_global"] > 0 else "G"
        
        return f"{vark_code}-{felder_code}"
    
    def get_all_hybrid_codes(self) -> List[str]:
        """Tüm 64 hibrit kod kombinasyonunu döndür"""
        codes = []
        vark_codes = ["V", "A", "R", "K"]
        felder_patterns = []
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tum_kodlar": self.get_all_hybrid_codes()[:10]  # İlk 10 örnek
        }
