...
**Son güncelleme:** 2026-04-23 — Chroma dörtlü ek smoke: GF37 clustering, GF47 recommendations, GF152 `duplicates/check`; GF38 + GF150  

Sütunlar: `Journey | API/Route | FE route | Son test (SHA) | Durum | Not`

| Journey | API/Route | FE route | Son test | Durum | Not |
|---------|-----------|----------|----------|-------|-----|
| J10–J13 Chroma | `api.v1.semantic_search`, clustering, recommendation, duplicate | TBD | `553bacf` + GF150/38/37/47/152 | **Yeşil (health+API smoke)** | Health: GF150. Arama: GF38. Clustering auto: GF37. `POST /api/v1/recommendations`: GF47. `POST /api/v1/duplicates/check` (admin): GF152. `PersistentClient` + dev tohum: `backend/scripts/chroma_seed_kiro2_questions.py`. |
