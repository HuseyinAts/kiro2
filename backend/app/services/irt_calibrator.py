...
def _em_3pl(
    response_vector: np.ndarray, a0: float = 1.0, b0: float = 0.0, c0: float = 0.25
    ...
    for iteration in range(EM_MAX_ITER):
        ...
        r_k = posterior.sum(axis=0) * (
            responses @ posterior / posterior.sum(axis=0).clip(1e-10)
        )
...
def _item_fit(
    responses: np.ndarray, a: float, b: float, c: float, n_groups: int = 10
    ...
    groups = [
        sorted_r[i : i + group_size]
        for i in range(0, n, group_size)
        if len(sorted_r[i : i + group_size]) >= 5
    ]
...
def calibrate_item(question_id: str, response_vector: np.ndarray) -> CalibrationResult:
    """
    ...
    if not result.is_acceptable:
        result.warning = (
            f"Parametre sınır dışı — clamp uygulandı: a={a:.3f}, b={b:.3f}, c={c:.3f}"
        )
    result.clamped()
