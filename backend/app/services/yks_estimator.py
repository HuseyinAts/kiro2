...
class TahminiNetler:
    ...
    toplam_tyt: float | None = None
    toplam_ayt: float | None = None
...
class YKSTahminRaporu:
    ...
    tyt: PuanTahmini | None
    say: PuanTahmini | None
    ea: PuanTahmini | None
    soz: PuanTahmini | None
    dil: PuanTahmini | None
...
def net_guvensizlik(
    ...
    """
    _net_merkez = theta_to_net(theta, ders, irt_a, irt_b, irt_c)
...
class YKSEstimator:
    ...
    def tyt_raporu(
        ...
        siralama_alt, _ = puan_to_siralama(
            puan_ust, "TYT"
        )  # yüksek puan → düşük sıralama
        siralama_ust, _ = puan_to_siralama(puan_alt, "TYT")

        ort_se = sum(dt.se for dt in ders_thetalar.values()) / max(
            len(ders_thetalar), 1
        )
    ...
    def tam_rapor(
        ...
        ayt_thetalar: dict[str, DersTheta] | None = None,
        hedef_puan_turu: str | None = None,
    ) -> YKSTahminRaporu:
    ...
    def tek_ders_katkisi(
        ...
        """
        # Mevcut durum
        tyt_thetalar = {k: v for k, v in diger_thetalar.items() if k in TYT_DERSLER}
        ayt_thetalar = {k: v for k, v in diger_thetalar.items() if k in AYT_DERSLER}
        ...
        rapor_oncesi = self.tam_rapor(
            tyt_thetalar, ayt_thetalar or None, hedef_puan_turu=puan_turu
        )
        ...
        rapor_sonrasi = self.tam_rapor(
            tyt_thetalar, ayt_thetalar or None, hedef_puan_turu=puan_turu
        )

        puan_once = getattr(
            rapor_oncesi, puan_turu.lower().replace("ö", "o"), rapor_oncesi.tyt
        )
        puan_sonra = getattr(
            rapor_sonrasi, puan_turu.lower().replace("ö", "o"), rapor_sonrasi.tyt
        )
        ...
        return {
            ...
            "siralama_degisimi": puan_sonra.tahmini_siralama
            - puan_once.tahmini_siralama,
        }
