...
async def tyt_tahmini(
    ...
    tyt_thetalar = {k: v for k, v in thetalar.items()
                    if k in ("turkce", "sosyal", "mat", "fen")}
...
async def ayt_tahmini(
    ...
    if puan_turu not in ("SAY", "EA", "SÖZ", "SOZ", "DİL", "DIL"):
        raise HTTPException(status_code=400,
                            detail="Geçersiz puan türü. SAY | EA | SÖZ | DİL")
    puan_turu = puan_turu.replace("SOZ", "SÖZ").replace("DIL", "DİL")

    thetalar = await _kullanici_thetalarini_cek(str(current_user.id), db)
    tyt_thetalar = {k: v for k, v in thetalar.items()
                    if k in ("turkce", "sosyal", "mat", "fen")}
    ayt_thetalar = {k: v for k, v in thetalar.items()
                    if k in ("mat", "fizik", "kimya", "biyoloji",
                             "edebiyat", "tarih1", "tarih2",
                             "cografya1", "cografya2", "felsefe", "din")}

    if not tyt_thetalar:
        raise HTTPException(status_code=404,
                            detail="TYT oturumu bulunamadı.")
...
async def tam_rapor(
    ...
    tyt_thetalar = {k: v for k, v in thetalar.items()
                    if k in ("turkce", "sosyal", "mat", "fen")}
    ayt_thetalar = {k: v for k, v in thetalar.items()
                    if k not in ("turkce", "sosyal", "fen")}

    if not tyt_thetalar:
        raise HTTPException(status_code=404,
                            detail="Henüz CAT oturumu bulunamadı.")

    est   = YKSEstimator()
    rapor = est.tam_rapor(tyt_thetalar,
                          ayt_thetalar if ayt_thetalar else None)
...
async def ders_katkisi(
    ...
    if mevcut is None:
        raise HTTPException(status_code=404,
                            detail=f"'{body.ders_kodu}' dersi için oturum yok.")
...
async def ders_thetalar(
    ...
    def theta_label(theta: float) -> str:
        ...
        for r, label in THETA_LABEL.items():
            if t10 in r: return label
        return "İleri" if theta > 1.5 else "Temel"

    DERS_ADI = {
        "mat": "Matematik", "turkce": "Türkçe", "fizik": "Fizik",
        "kimya": "Kimya", "biyoloji": "Biyoloji", "fen": "Fen Bilimleri",
        "sosyal": "Sosyal Bilimler", "edebiyat": "Edebiyat",
