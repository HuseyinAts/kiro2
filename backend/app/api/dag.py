...
async def check_topic_readiness(
    ...
    result = await svc.check_can_study_topic(
        str(current_user.id), topic_id
    )
    return {
...
async def get_learning_path(
    ...
    path = await svc.get_learning_path_for_user(
        str(current_user.id), topic_id
    )
    return {
...
async def get_next_topic(
    ...
    next_tid = await svc.get_next_recommended_topic(
