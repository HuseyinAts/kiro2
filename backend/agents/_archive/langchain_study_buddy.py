...
class LangChainStudyBuddy:
    ...
    def _create_tools(self):
        ...
        def solve_math(expression: str) -> str:
            ...
            try:
                ...
                # Sadece sayılar ve operatörlere izin ver
    ...
    def _create_agent(self):
        ...
        # Create agent based on available LLM using modern patterns
        if self.llm_service.chat_model:
            # Use modern OpenAI Functions Agent pattern
            try:
                self.agent = create_openai_functions_agent(
                    llm=self.llm_service.chat_model, tools=self.tools, prompt=prompt
                )
            except Exception as e:
                logger.warning(f"OpenAI functions agent failed: {e}, using structured chat")
                # Fallback to structured chat agent
                self.agent = create_structured_chat_agent(
                    llm=self.llm_service.chat_model, tools=self.tools, prompt=prompt
                )
        else:
            # Use React agent for other LLMs
