...
class DataExportRequest(BaseModel):
    ...
    data_categories: Optional[List[str]] = None  # None = all data
...
async def request_data_export(
    ...
    try:
        ...
        stmt = select(KVKKDataExportRequest).where(
            KVKKDataExportRequest.user_id == current_user.id,
        ...
        logger.error("data_export_request_error", user_id=current_user.id, error=str(e))
        await db.rollback()
        raise HTTPException(
...
async def get_export_requests(
    ...
    try:
        stmt = (
            ...
            .where(KVKKDataExportRequest.user_id == current_user.id)
        ...
        logger.error("get_export_requests_error", user_id=current_user.id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def request_data_deletion(
    ...
    try:
        ...
        stmt = select(KVKKDataDeletionRequest).where(
            KVKKDataDeletionRequest.user_id == current_user.id,
        ...
        logger.error("data_deletion_request_error", user_id=current_user.id, error=str(e))
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_deletion_requests(
    ...
    try:
        stmt = (
            ...
            .where(KVKKDataDeletionRequest.user_id == current_user.id)
        ...
        logger.error("get_deletion_requests_error", user_id=current_user.id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
