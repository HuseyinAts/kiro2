...
class MetadataResponse(BaseModel):
    """Metadata yanıtı"""
    kitap: Optional[DetectionResponse] = None
    test_no: Optional[DetectionResponse] = None
    sayfa: Optional[DetectionResponse] = None
    konu: Optional[DetectionResponse] = None
    cevaplar: Optional[DetectionResponse] = None
    cozum: Optional[DetectionResponse] = None
...
def get_detector() -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="YOLO modülü yüklenemedi. Ultralytics kurulu mu?"
        )
...
async def detect_questions_batch(
    ...
    if len(files) > 20:
        raise HTTPException(
            status_code=400,
            detail="Maksimum 20 dosya yüklenebilir."
        )
