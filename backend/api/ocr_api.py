...
class Base64ImageRequest(BaseModel):
    ...
    engine: str | None = Field(
        None, description="OCR engine: easyocr, paddleocr, tesseract, claude_vision"
    )
...
def get_engine_from_string(engine_str: str | None) -> OCREngine | None:
...
def ocr_result_to_response(result: OCRResult) -> OCRResultResponse:
    ...
    return OCRResultResponse(
        ...
        boxes=[
            OCRBoxResponse(
                text=box.text, confidence=box.confidence, bbox=list(box.bbox)
            )
...
async def process_question_base64(
    request: Base64ImageRequest,
    current_user: User = Depends(get_current_user),
):
    """
...
async def yolo_detect_and_ocr(
    request: YOLOOCRRequest,
    current_user: User = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        detection_result = detector.detect(
            img, confidence_threshold=request.confidence_threshold
        )
