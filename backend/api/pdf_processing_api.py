...
class PDFProcessingConfig(BaseModel):
    ...
    auto_detect_metadata: bool = Field(
        default=True, description="Auto-detect exam metadata"
    )
...
def process_pdf_task(
    job_id: str, file_path: str, config: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        processing_time = (
            datetime.now() - processing_jobs[job_id]["started_at"]
        ).total_seconds()
        ...
        logger.info(
            f"PDF processing completed: {job_id} - {len(filtered_questions)} questions extracted"
        )

        return result

    except Exception as e:
        logger.error(f"PDF processing failed: {job_id} - {e!s}")
...
async def upload_pdf(
    ...
    auto_detect_metadata: bool = Query(
        default=True, description="Auto-detect metadata"
    ),
    min_confidence: float = Query(
        default=0.5, ge=0.0, le=1.0, description="Min confidence"
    ),
) -> PDFUploadResponse:
    """
...
async def get_processing_status(
    job_id: str, current_user: AuthenticatedUser = Depends(get_current_user)
) -> PDFProcessingStatus:
    """
...
async def pdf_api_health() -> dict[str, Any]:
    ...
    return {
        "status": "healthy"
        if (analyzer_healthy and upload_dir_writable and celery_healthy)
        else "degraded",
