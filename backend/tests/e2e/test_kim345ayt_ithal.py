"""345 2025 AYT Kimya ithalinin kapilari.
...
BOLUMLER
1. VERI SETI BUTUNLUGU  -- 1304 kayit, 5 sik, dolu cevap, benzersiz id.
2. YAPISAL GARANTILER   -- 160 test, basili numara == test ici sira,
                           metin kanalina cevap sizmamis.
3. KONU BAGLANTISI      -- her kayit KIM-345A25 agacinda (konu ya da unite);
                           migration ile ayni kodlar.
4. SOZLESME             -- kaynak adi ASCII, kayitli, cakismasiz; eski hat
                           yazimi 0050 ile duzeltiliyor.
...
from scripts.kitap import kim345ayt_ithal as mi  # noqa: E402
...
ITHAL_YOLU = KOK / "scripts" / "kitap" / "kim345ayt_ithal.py"
AGAC_YOLU = KOK / "alembic" / "versions" / "0049_kim345ayt_konu_agaci.py"
AD_YOLU = KOK / "alembic" / "versions" / "0050_kim345ayt_kaynak_adi.py"
ARAC_YOLLARI = [
    ITHAL_YOLU,
    AGAC_YOLU,
    AD_YOLU,
    KOK / "alembic" / "versions" / "0051_kim345ayt_eski_hat_cevap.py",
    KOK / "scripts" / "kitap" / "kim345ayt_tarama.py",
    KOK / "scripts" / "kitap" / "kim345ayt_kutu.py",
    KOK / "scripts" / "kitap" / "kim345ayt_kirp.py",
    KOK / "scripts" / "kitap" / "kim345ayt_metin_harness.py",
]

BEKLENEN_SORU = 1304
BEKLENEN_TEST = 160
# 50 konu (Kazanim Odakli) + 12 unite (OSYM Tadinda / Orijinal) dugumu
BEKLENEN_DUGUM = 62
DUZEY = {"konu": 834, "unite": 470}
BAYRAKLAR = {
    "kaynak_kusuru": 278,
    "cikmis_soru": 94,
...
@pytest.fixture(scope="module")
def agac() -> object:
    return _modul("agac0049", AGAC_YOLU)


@pytest.fixture(scope="module")
def ad_duzeltme() -> object:
    return _modul("ad0050", AD_YOLU)
...
def test_test_ve_ici_sira(kayitlar: list[dict]) -> None:
    ...
    assert len(ortulu) == BAYRAKLAR["numara_ortulu"]
    for k in ortulu:
        pm = k["pipeline_metadata"]
        assert (
...
def test_konu_kodlari_migration_ile_ayni(
    ...
    assert {(u, k) for u, k, _ in konu} == {
        (f"KIM-345A25-U{x['unite']:02d}", x["kod"]) for x in h["konular"]
    }
    # adlar haritanin ASCII'ye katlanmis haliyle ayni (buyuk/kucuk harf haric)
    ad_ascii = {x["kod"]: x["ad_ascii"] for x in h["uniteler"] + h["konular"]}
    for kod, ad in [*unite, *((k, a) for _, k, a in konu)]:
        assert ad.isascii() and ad.upper() == ad_ascii[kod], kod
...
def test_eslesme_duzeyi_test_turuyle(kayitlar: list[dict]) -> None:
    """Kazanim Odakli -> konu dugumu; OSYM Tadinda / Orijinal -> unite dugumu."""
    say: Counter[str] = Counter()
    for k in kayitlar:
        pm = k["pipeline_metadata"]
        beklenen = "konu" if pm["test_turu"] == "kazanim_odakli" else "unite"
...
def test_migration_zinciri(agac: object, ad_duzeltme: object) -> None:
    assert agac.down_revision == "0048_mat345ayt_ikinci_okuma"  # type: ignore[attr-defined]
...
def test_eski_hat_yazimi_ayni_kitap_0050_duzeltiyor(ad_duzeltme: object) -> None:
    """Eski hat yazimi ayni anahtara cozuluyor; 0050 onu yeni ada ceviriyor."""
    ...
    for baska in (
        "345 2024 Ayt Kimya Soru Bankas\u0131",
        "345 2025 Tyt Kimya Soru Bankas\u0131",
        "345 Tyt Kimya Soru Bankas\u0131",
    ):
        assert normalize_anahtar(baska) != normalize_anahtar(yeni), baska
...
def test_ithal_pasif_sozlesmesi() -> None:
    ...
    assert "'AYT', 'KIMYA'" in mi._QM and "'PENDING'" in mi._QM
...
def test_test_konusu_haritada_yoksa_durur(paket: dict) -> None:
    p = copy.deepcopy(paket)
    p["harita"]["testler"][0]["konu"] = "KIM-345A25-U99-01"
