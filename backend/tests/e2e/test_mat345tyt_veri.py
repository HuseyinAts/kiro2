"""345 2025 TYT Matematik veri hattinin koruma testleri.
...
NE KORUR
--------
1. CEVAP ANAHTARI  -- 2063 cevap, 202 test, okuma kanallari, numara surekliligi.
2. KONU HARITASI   -- 13 bolum / 30 konu (icindekiler), 202 test tek aralikta,
...
5. METIN           -- 2063 kayit, basili numara == test ici sira (tek
                      istisna disk altinda), bes sik, cevap alani yok.
...
BEKLENEN_SORU = 2063
BEKLENEN_TEST = 202
BEKLENEN_BOLUM, BEKLENEN_KONU = 13, 30
SORU_SAYFASI = 399
AYRAC = {5, 57, 85, 111, 127, 169, 203, 265, 295, 327, 353, 381, 401}
KAYNAK_DAGILIMI = {
...
HARF_DAGILIMI = {"A": 404, "B": 401, "C": 473, "D": 413, "E": 372}
TUR_TEST = {"klasiklesmis": 95, "osym_tadinda": 90, "orijinal": 17}
TUR_SORU = {"klasiklesmis": 1337, "osym_tadinda": 653, "orijinal": 73}
BANT_KONUSUZ = 18
SIMGE_TOPLAM, NUMARA_TOPLAM = 2213, 2076
SUTUN_KANALI = {"numara": 796, "simge": 2}
SAYFA_ALTI = 886
CEVAP_SATIRI_UST = 889
KART_G, KART_Y = 742, 977
ORTME_SORU = 167
SEKILLI, GORSEL_SIKLI, KUSURLU, ETIKETLI = 695, 22, 101, 149
DUZELTME_OKUMASI = 18
NUMARASI_ORTULU = ["MAT345-T192_07"]
GUCLU_MUKERRER = 27
...
def test_konu_araliklari_sirali_ve_ayraci_atliyor(harita: dict) -> None:
    ...
    for k in kon:
        ...
        assert k["kod"].startswith(f"MAT-345T25-B{k['bolum']:02d}-")
...
def test_kesim_metne_degen_kutular_civili(kutu: dict) -> None:
    """Ust kesimi +-3 satirda metne degen 3 kutu gozle incelendi (bkz. yontem notu)."""
...
def test_ortme_olcumu_tam_kitap(ortme: dict) -> None:
    ...
    # Tek kalan: s396 sol, OSYM cercevesinin 6 piksellik cikintisi, renk esigin
    # 1 birim disinda (L1 46 > 45); gozle bakildi, metin degil.
    assert ortme["kenar_kapisi_ihlali"] == len(ortme["kenar"]) == 1
    assert [(k["birim"], k["soru"], k["sol"], k["sag"]) for k in ortme["kenar"]] == [
        ("MAT345-T193", 1, 6, 0)
    ]
