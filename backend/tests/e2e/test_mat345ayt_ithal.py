"""345 2025 AYT Matematik ithalinin kapilari.
...
BOLUMLER
1. VERI SETI BUTUNLUGU  -- 1942 kayit, 5 sik, dolu cevap, benzersiz id.
2. YAPISAL GARANTILER   -- 187 test, basili numara == test ici sira,
                           metin kanalina cevap sizmamis.
3. KONU BAGLANTISI      -- her kayit MAT-345A25 agacinda; migration ile ayni kodlar.
4. SOZLESME             -- kaynak adi ASCII, kayitli, cakismasiz; eski hat
                           yazimi 0046 ile duzeltiliyor.
...
AD_YOLU = KOK / "alembic" / "versions" / "0046_mat345ayt_kaynak_adi.py"
ARAC_YOLLARI = [
    ITHAL_YOLU,
    AGAC_YOLU,
    AD_YOLU,
    KOK / "scripts" / "kitap" / "mat345ayt_tarama.py",
    KOK / "scripts" / "kitap" / "mat345ayt_kutu.py",
    KOK / "scripts" / "kitap" / "mat345ayt_kirp.py",
    KOK / "scripts" / "kitap" / "mat345ayt_metin_harness.py",
]
...
SINAV = {"AYT": 122, "LYS": 19, "YGS": 1}
SAYFA_ILK, SAYFA_SON = 6, 384
...
@pytest.fixture(scope="module")
def agac() -> object:
    return _modul("agac0045", AGAC_YOLU)


@pytest.fixture(scope="module")
def ad_duzeltme() -> object:
    return _modul("ad0046", AD_YOLU)
...
def test_test_ve_ici_sira(kayitlar: list[dict]) -> None:
    ...
    for k in ortulu:
        pm = k["pipeline_metadata"]
        assert (
            pm["kirpim_capa_kanali"] == "simge" or pm["kirpim_numara_disk_ortulu"]
        ), k["id"]
    for k in kayitlar:
        pm = k["pipeline_metadata"]
...
def test_konu_kodlari_migration_ile_ayni(
    ...
    assert {(u, k) for u, k, _ in konu} == {
        (f"MAT-345A25-B{x['bolum']:02d}", x["kod"]) for x in h["konular"]
    }
...
def test_migration_zinciri(agac: object, ad_duzeltme: object) -> None:
    assert agac.down_revision == "0044_mat345_eski_hat_cevap"  # type: ignore[attr-defined]
...
def test_eski_hat_yazimi_ayni_kitap_0046_duzeltiyor(ad_duzeltme: object) -> None:
    """Eski hat yazimi ayni anahtara cozuluyor; 0046 onu yeni ada ceviriyor."""
    ...
    assert normalize_anahtar(
        "345 2024 Ayt Matematik Soru Bankas\u0131"
...
def test_ithal_pasif_sozlesmesi() -> None:
    ...
    assert "'AYT', 'MATEMATIK'" in mi._QM and "'PENDING'" in mi._QM
...
def test_on_kontrol_numarasiz_satir_numara_capali_ise_durur(paket: dict) -> None:
    """Basili numara yalniz disk altinda (simge capasi / numara_disk_ortulu) bos olabilir."""
...
def test_test_konusu_haritada_yoksa_durur(paket: dict) -> None:
    p = copy.deepcopy(paket)
    p["harita"]["testler"][0]["konu"] = "MAT-345A25-B99-01"
