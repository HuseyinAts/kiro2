...
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/summary\"",
    "CreateSummaryCommand",
    "request=request, persist_file=persist_file, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.put\(\"/summary/{entry_id}\"",
    "UpdateSummaryCommand",
    "entry_id=entry_id, request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.delete\(\"/summary/{entry_id}\"",
...
content = replace_endpoint_body(
    content,
    r"@router.put\(\"/goals/{goal_id}\"",
    "UpdateGoalCommand",
    "goal_id=goal_id, request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.patch\(\"/goals/{goal_id}/progress\"",
    "UpdateGoalProgressCommand",
    "goal_id=goal_id, request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/goals/{goal_id}/adjust\"",
    "AdjustGoalCommand",
    "goal_id=goal_id, reason=reason, new_target_value=new_target_value, new_target_date=new_target_date, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/goals/{goal_id}/retrospective\"",
    "CreateGoalRetrospectiveCommand",
    "goal_id=goal_id, lessons_learned=lessons_learned, success_factors=success_factors, challenges_faced=challenges_faced, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.delete\(\"/goals/{goal_id}\"",
    "DeleteGoalCommand",
    "goal_id=goal_id, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/insights/analyze\"",
    "AnalyzeEntriesForInsightsCommand",
    "start_date=start_date, end_date=end_date, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.delete\(\"/insights/{insight_id}\"",
    "DeleteInsightCommand",
    "insight_id=insight_id, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/reflection\"",
    "CreateReflectionCommand",
    "request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/learning\"",
    "CreateLearningEntryCommand",
    "request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/learning/{entry_id}/review\"",
    "RecordReviewCommand",
    "entry_id=entry_id, remembered=remembered, quality=quality, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/learning/{entry_id}/link\"",
    "LinkConceptsCommand",
    "entry_id=entry_id, concepts=concepts, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/emotional\"",
    "TrackEmotionalStateCommand",
    "request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/export\"",
    "CreateExportCommand",
    "request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/export/share\"",
    "CreateShareLinkCommand",
    "request=request, user_id=current_user.id, db=db",
)
content = replace_endpoint_body(
    content,
    r"@router.post\(\"/backup/encrypted\"",
    "CreateEncryptedBackupCommand",
    "password=password, user_id=current_user.id, db=db",
)
