...
async def _verify_ownership(
    db: AsyncSession, model_class, entity_id: UUID, user_id: UUID, label: str = "Kayit"
):
...
def _goal_to_response(goal) -> GoalResponse:
    ...
    return GoalResponse(
        ...
        days_remaining=getattr(goal, "days_remaining", 0),
...
class CreateSummaryCommandHandler(
    CommandHandler[CreateSummaryCommand, DiaryEntryResponse]
):
    async def handle(self, command: CreateSummaryCommand) -> DiaryEntryResponse:
        ...
        if existing:
            raise HTTPException(
                status_code=400,
                detail=f"{command.request.date} tarihi icin kayit zaten mevcut",
            )
        entry = await service.generate_summary(
            user_id=command.user_id,
            entry_date=command.request.date,
            tasks=command.request.tasks,
            persist_file=command.persist_file,
        )
...
class UpdateSummaryCommandHandler(
    CommandHandler[UpdateSummaryCommand, DiaryEntryResponse]
):
    async def handle(self, command: UpdateSummaryCommand) -> DiaryEntryResponse:
        await _verify_ownership(
            command.db, DiaryEntry, command.entry_id, command.user_id, "Ozet"
        )
        service = DiaryService(command.db)
        entry = await service.update_summary(
            command.entry_id,
            highlights=command.request.highlights,
            learnings=command.request.learnings,
            challenges=command.request.challenges,
        )
        if not entry:
...
class DeleteSummaryCommandHandler(
    CommandHandler[DeleteSummaryCommand, SuccessResponse]
):
...
class UpdateGoalCommandHandler(CommandHandler[UpdateGoalCommand, GoalResponse]):
    async def handle(self, command: UpdateGoalCommand) -> GoalResponse:
        await _verify_ownership(
            command.db, Goal, command.goal_id, command.user_id, "Hedef"
        )
        service = GoalService(command.db)
...
class UpdateGoalProgressCommandHandler(
    CommandHandler[UpdateGoalProgressCommand, dict[str, Any]]
):
...
class AdjustGoalCommandHandler(CommandHandler[AdjustGoalCommand, GoalResponse]):
    async def handle(self, command: AdjustGoalCommand) -> GoalResponse:
        ...
        goal = await service.adjust_goal(
            command.goal_id,
            command.reason,
            command.new_target_value,
            command.new_target_date,
        )
        if not goal:
            raise HTTPException(status_code=404, detail="Hedef bulunamadi")
        return _goal_to_response(goal)
...
class CreateGoalRetrospectiveCommandHandler(
    CommandHandler[CreateGoalRetrospectiveCommand, GoalResponse]
):
    async def handle(self, command: CreateGoalRetrospectiveCommand) -> GoalResponse:
        ...
        goal = await service.create_retrospective(
            ...
            command.success_factors,
            command.challenges_faced,
        )
        if not goal:
            raise HTTPException(status_code=404, detail="Hedef bulunamadi")
        return _goal_to_response(goal)
...
class AnalyzeEntriesForInsightsCommandHandler(
    CommandHandler[AnalyzeEntriesForInsightsCommand, list[InsightResponse]]
):
    async def handle(
        self, command: AnalyzeEntriesForInsightsCommand
...
class DeleteInsightCommandHandler(
    CommandHandler[DeleteInsightCommand, SuccessResponse]
):
    async def handle(self, command: DeleteInsightCommand) -> SuccessResponse:
        await _verify_ownership(
            command.db, Insight, command.insight_id, command.user_id, "Insight"
        )
        service = InsightService(command.db)
...
class CreateReflectionCommandHandler(
    CommandHandler[CreateReflectionCommand, ReflectionResponse]
):
...
class CreateLearningEntryCommandHandler(
    CommandHandler[CreateLearningEntryCommand, LearningEntryResponse]
):
    async def handle(
        self, command: CreateLearningEntryCommand
    ) -> LearningEntryResponse:
...
class RecordReviewCommandHandler(
    CommandHandler[RecordReviewCommand, LearningReviewResponse]
):
    async def handle(self, command: RecordReviewCommand) -> LearningReviewResponse:
        ...
        result = await service.record_review(
            command.entry_id, command.remembered, command.quality
        )
        if not result:
...
class LinkConceptsCommandHandler(CommandHandler[LinkConceptsCommand, SuccessResponse]):
    async def handle(self, command: LinkConceptsCommand) -> SuccessResponse:
        ...
        return SuccessResponse(
            success=True, message=f"{len(command.concepts)} kavram baglandi"
        )
...
class TrackEmotionalStateCommandHandler(
    CommandHandler[TrackEmotionalStateCommand, EmotionalStateResponse]
):
    async def handle(
        self, command: TrackEmotionalStateCommand
    ) -> EmotionalStateResponse:
...
class CreateShareLinkCommandHandler(
    CommandHandler[CreateShareLinkCommand, ShareLinkResponse]
):
    async def handle(self, command: CreateShareLinkCommand) -> ShareLinkResponse:
        service = ExportService(command.db)
        share = await service.create_share_link(
            command.user_id, command.request.export_id, command.request.expires_in_days
        )
        if not share:
...
class CreateEncryptedBackupCommandHandler(
    CommandHandler[CreateEncryptedBackupCommand, dict[str, Any]]
):
    async def handle(self, command: CreateEncryptedBackupCommand) -> dict[str, Any]:
        service = ExportService(command.db)
        backup = await service.create_encrypted_backup(
            command.user_id, command.password
        )
        return {
            "success": True,
