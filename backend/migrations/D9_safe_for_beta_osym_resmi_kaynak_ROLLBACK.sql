-- D9_safe_for_beta_osym_resmi_kaynak_ROLLBACK.sql
-- Geri alma: D9'un ekledigi 'osym_resmi_kaynak' dalini kaldirir, view'i
-- D9 calistirilmadan onceki (10 Eyl 2026'da pg_get_viewdef ile olculen)
-- canli haline dondurur. Bu dosya calistirilmadan once, 0011_osym_aktiflestirme
-- migration'inin downgrade()'i cagrilmis olmali (aksi halde is_active=true
-- OSYM sorulari view disina, ama mv_safe_for_beta REFRESH edilene kadar
-- eski matview'da gorunmeye devam eder -- bkz D9 dosyasindaki bayat pencere notu).
...
CREATE OR REPLACE VIEW v_safe_for_beta AS
 SELECT id,
  ...
  WHERE (quality_review_status::text = ANY (ARRAY['human_verified'::character varying::text, 'auto_judged_high'::character varying::text]))
    ...
    AND pipeline_metadata IS NOT NULL AND ((pipeline_metadata::jsonb ->> 'student_coherent'::text) = 'true'::text OR pipeline_metadata::jsonb ? 'verified_provisional'::text OR pipeline_metadata::jsonb ? 'consensus_2signal_run'::text OR pipeline_metadata::jsonb ? 'math_promote_run'::text OR pipeline_metadata::jsonb ? 'verbal_promote_run'::text)
    AND (is_ai_generated = false OR review_status::text = 'APPROVED'::text);

COMMENT ON VIEW v_safe_for_beta IS
'Beta-safe questions (D9 geri alindi, 10 Eyl 2026). Bkz: backend/migrations/D9_safe_for_beta_osym_resmi_kaynak_ROLLBACK.sql.';
