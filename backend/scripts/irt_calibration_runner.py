...
from app.services.irt_calibrator import calibrate_item, MIN_RESPONSES_CTT, MIN_RESPONSES_3PL
...
def quality_score(result) -> float:
    s = 0.0
    if result.converged:         s += 0.40
    if result.item_df > 0 and (result.item_chi2 / result.item_df) < 2.0: s += 0.30
    if result.is_acceptable:     s += 0.20
    if result.n_responses >= 200: s += 0.10
    return round(s, 2)
...
def main():
    ...
    if not filtered:
        print("Esigi gecen soru yok."); conn.close(); return
    ...
    for i, (qid, subj_area, old_a, old_b, old_c, was_cal, n_resp) in enumerate(filtered):
        ...
        print(f"[{i+1:>4}/{len(filtered)}] {status_icon} "
              f"| n={res.n_responses:>4} | "
              f"a={res.a:.3f} b={res.b:+.3f} c={res.c:.3f} "
              f"| conv={'Y' if res.converged else 'N'} "
              f"| fit={res.item_chi2/res.item_df:.1f}" if res.item_df else
              f"[{i+1:>4}/{len(filtered)}] {status_icon} | n={res.n_responses:>4} | "
              f"a={res.a:.3f} b={res.b:+.3f} c={res.c:.3f}"
              f" ({dt}s) {res.warning[:40] if res.warning else ''}")

        if res.method == "3pl_em":   ok3pl += 1
        elif res.method == "ctt_fallback": ok_ctt += 1
        else: skipped += 1

        if not args.dry_run and res.method in ("3pl_em", "ctt_fallback"):
            try:
                cur.execute(UPDATE_SQL, dict(
                    a=round(res.a,4), b=round(res.b,4), c=round(res.c,4),
                    calibrated=res.converged, n=res.n_responses,
                    qs=qs, qid=qid
                ))
                cur.execute(INSERT_HISTORY_SQL, dict(
                    qid=qid, method=res.method, n=res.n_responses,
                    old_a=old_a, old_b=old_b, old_c=old_c,
                    new_a=round(res.a,4), new_b=round(res.b,4), new_c=round(res.c,4),
                    **ci
                ))
                if (i+1) % 20 == 0:
                    conn.commit()
    ...
    print(f"Süre: {total_time}s | {'DRY RUN — DB YAZILMADI' if args.dry_run else 'DB GÜNCELLENDİ ✅'}")
    if args.dry_run:
