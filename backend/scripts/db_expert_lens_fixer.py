...
def analyze_and_fix_models():
    ...
    for filepath in model_files:
        ...
        while True:
            ...
            end_idx = parse_balanced_parentheses(
                content, idx + len("mapped_column") - 1
            )
            if end_idx != -1:
                inside = content[idx + len("mapped_column(") : end_idx]
                # Check if it contains Text, JSON, JSONB (make sure it's the type)
                if (
                    re.search(r"\b(Text|JSON|JSONB)\b", inside)
                    and "deferred=" not in inside
                    and "deferred =" not in inside
                ):
        ...
        content = re.sub(
            r"is_active:\s*Mapped\[bool\]\s*=\s*mapped_column\(([^)]*)",
            replacer_index,
            content,
        )
