"""Wave-1 splitter: master.csv -> answer-free batch files for blind solver agents.

The 'key' (DB correct_answer) is STRIPPED from solver input so blindness is
enforced structurally: solver agents physically cannot see the answer key.
master.csv is a single-column CSV where each row is one JSON object.
"""
