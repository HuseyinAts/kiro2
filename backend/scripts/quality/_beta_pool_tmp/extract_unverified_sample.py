"""Beta pool büyütme — unverified havuzundan stratified pilot sample çek.

Her subject_area için ~N soru (rastgele, deterministik seed=md5). Workflow
LLM-as-judge ajanlarının okuyacağı JSONL üretir (tam içerik: soru + şıklar +
cevap + açıklama). SALT OKUNUR (sadece SELECT).
"""
...
PER_SUBJECT = int(os.environ.get("PER_SUBJECT", "30"))
OUT = os.path.join(os.path.dirname(__file__), "unverified_sample.jsonl")
...
WITH ranked AS (
  ...
  FROM question_bank
  WHERE is_active = TRUE AND quality_review_status = 'unverified'
