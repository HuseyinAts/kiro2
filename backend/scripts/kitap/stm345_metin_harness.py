#!/usr/bin/env python
"""345 2025 Start Matematik: transkripsiyon harness'i (hazirla / topla / kapi).
...
okumayi yapmaz, etrafini kurar (mat345tyt_metin_harness.py deseni):
...
---------------------------------------
1. Her kirpim tam bir kez okunmus olmali (371 dosya, eksik/fazla yok).
2. Okunan BASILI numara == test ici sira (kitabin cevap satiri
   ...
   ve capasi bu yuzden simgeden alinan kutular (kirpim kutularinda
   capa_kanali == 'simge'); orada okuyucu null yazabilir, sayi yazarsa yine
   esit olmali.
3. Her soruda bes sik (A-E) bos olmayan metin tasimali.
4. Toplam soru sayisi 371.

ORTME BILDIRIMI
---------------
hazirla, okuyucu diski halkasinda kitap murekkebi olculen sorulari
(`345_2025_start_matematik_ortme_olcumu.json`) grup listesinin yanina
`ortme_NN.txt` olarak yazar; okuyucu o sorularda disk kenarindaki
harflerin gorunup gorunmedigini `kaynak_kusuru`na yazar.

KULLANIM
--------
    python backend/scripts/kitap/stm345_metin_harness.py hazirla
    python backend/scripts/kitap/stm345_metin_harness.py topla
    python backend/scripts/kitap/stm345_metin_harness.py kapi
"""
...
ORTME = CIKTI / "345_2025_start_matematik_ortme_olcumu.json"
HEDEF = CIKTI / "345_2025_start_matematik_metin.json"
KIRPIM = KOK / "backend" / "_p345_gecici" / "start" / "kirpim"
OKUMA_KOKU = Path(r"C:\Users\husey\VeraFilm\s_okuma_kirpim")
PARCA_DIZINI = Path(r"C:\Users\husey\VeraFilm\s_metin_parca")
BEKLENEN_SORU = 371
GRUP_SORU = 36
OLCEK = 2
...
def ortme_listesi() -> set[str]:
    """Okuyucu diski halkasinda kitap murekkebi olculen sorular (stm345_kirp)."""
    return {
        _ad(o["birim"], o["soru"])
        ...
        json.dumps(
            {
                ...
                "nereden": (
                    "Soru kirpimlarindan okundu (stm345_kirp.py, 2x Lanczos). Sorular "
