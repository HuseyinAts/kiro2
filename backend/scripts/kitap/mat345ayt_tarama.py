#!/usr/bin/env python
"""345 2025 AYT Matematik: capa taramasi (okuyucu simgesi + basili soru numarasi + ara cizgi).

Kirpim kutulari iki BAGIMSIZ piksel kanalindan birine dayanir; hangisinin
kullanilacagini kitabin kendi cevap satiri (sutun basina girdi sayisi) secer
(`mat345ayt_kutu.py`). Bu script yalniz olcer, karar vermez.
...
(240,238,247, L1 < 30). Ilk surum disk+glif BIRLESIK bilesenini boyutla
suzuyordu: kitabin 'UcDortBes' filigrani disk rengine yakin oldugu icin
filigranla ortusen disk buyuk bilesene donustu ve simge kacti (s15 sag 8).
Kapi uyumu 769 -> 784 / 798 sutun.
...
KULLANIM
--------
    python backend/scripts/kitap/mat345ayt_tarama.py
"""
...
HEDEF = CIKTI / "345_2025_ayt_matematik_capa_taramasi.json"
...
def kaynak_dizin() -> Path:
    ...
    adaylar = [
        ...
        if p.is_dir() and p.name.startswith("345 2025 Ayt Matematik Soru Bankas")
    ]
    if len(adaylar) != 1:
...
def main() -> None:
    ...
    veri = {
        ...
        "arac": "scripts/kitap/mat345ayt_tarama.py",
        "kart": list(KART),
