"""345 2025 TYT Matematik Soru Bankasi -- PASIF ithal.

KAYNAK VE TAVANI
----------------
FERNUS okuyucusunun 1920x1080 ekran goruntuleri; sayfa karti (589, 43) -
(1331, 1020) = 742x977. 416 PNG, 399 soru sayfasi; basili sayfa numarasi =
dosya numarasi (sayfa altindaki basili numara ile olculdu).

BIRIM = TEST
------------
Her soru sayfasinin altinda sutun basina cevap satiri var; numara kitap
boyunca 202 kez 1'e donuyor ve her donus kitabin baslik bandindaki bir
...
(A 5x sirali, B 7x ters sirali) okundu, 2063/2063 ayni; ucuncu kanal piksel
en-yakin-komsu (LOO 1801/1817), uyumsuzlarin ve okuyucu tereddutlerinin
hepsi goz ile. Hicbir soru cozulmedi.

KONU AGACI
----------
Kitabin kendi icindekiler sayfalari (dosya 3-4): 13 bolum + 30 konu (0042,
MAT kokunun altinda MAT-345T25 oneki). Her test tek bir icindekiler
araliginda (202/202); sorular KONU dugumune baglanir.

CIKMIS SORULAR
--------------
'CIKMIS SORU' kutusundaki basili etiket (ornek 'MSU - 2021', 'TYT - 2023')
...
kalan 1 soru (T192_07) `numara_ortulu` bayragi tasir.

MUKERRER ADAYLARI ISARETLENIR, SILINMEZ
---------------------------------------
345_2025_tyt_matematik_mukerrer_adaylari.json: govde kelime kumesi
...
Detay: veriseti/zkitap/cikti/MAT_345_TYT_YONTEM.md
"""
...
ONEK_DOSYA = f"{CIKTI}/345_2025_tyt_matematik"
...
KAYNAK_ADI = "345 2025 TYT Matematik Soru Bankasi"
ONEK = KAYNAK_KAYITLARI[KAYNAK_ADI]["onek"]
MAT_KOK_KODU = "MAT"
KOD_ONEKI = "MAT-345T25"
SINIF_DUZEYI = 12
ITHAL_ARACI = "scripts/kitap/mat345tyt_ithal.py"
CROP_ONEK = "MAT345_TYT"
TELIF_NOTU = (
...
URETIM_NOTU = (
    ...
    "alti cevap satirindan iki bagimsiz gorsel okumayla alindi (2063/2063 ayni; "
    "piksel en-yakin-komsu ucuncu kanal; uyumsuz/tereddutlu girdiler goz ile). "
    "Sorular cozulmedi. Kirpim kutulari basili soru numarasindan (796 sutun) ya "
    "da okuyucu simgesinden (2 sutun) turetildi. Transkripsiyon 33 grupta ayri "
    "okuyucularla kirpimdan yapildi; anahtar gosterilmedi. Detay: "
    "veriseti/zkitap/cikti/MAT_345_TYT_YONTEM.md"
)
ANAHTAR_DOGRULAMASI = (
    "sayfa_alti_satir_iki_okuma_2063_2063_ayni__piksel_nn_loo_1801_1817__"
    "uyumsuz_17_tereddut_44_kapsam_disi_63_yarim_satir_goz__"
    "numara_1_ya_da_onceki_arti_1_202_test__"
    "basili_numara_esittir_test_ici_sira_2062_2063_biri_disk_altinda"
)
...
# 'MSU - 2021' / 'TYT - 2023' / '2019 - TYT': sinav adi + yil (U+00DC = U-umlaut).
...
def kayit_uret(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pipeline_metadata": {
            ...
            "metin_kaynagi": "soru_kirpimi_gorsel_okuma_33_grup",
    ...
    *,
    ...
    for s in veri["sorular"]:
        ...
        if s["basili_no"] != sira_i and not (
            s["basili_no"] is None and k["capa_kanali"] == "simge"
        ):
...
def _on_kontrol(kayitlar: list[dict[str, Any]]) -> list[str]:
    ...
    for k in kayitlar:
        ...
        if pm["soru_no_basili"] != pm["birim_ici_sira"] and not (
            pm["soru_no_basili"] is None and pm["kirpim_capa_kanali"] == "simge"
        ):
...
def _konu_id_ata(
    ...
    if eksik:
        return (
            ...
            "0042_mat345tyt_konu_agaci kosmadi mi? Kok dugume dusurup "
