#!/usr/bin/env python
"""345 2025 Paragraf Sifir Risk: capa taramasi (okuyucu simgesi + basili soru numarasi + ara cizgi).
...
(`prg345_kutu.py`). Bu script yalniz olcer, karar vermez.

KANAL 1 -- FERNUS OKUYUCU SIMGESI (buyutec)
-------------------------------------------
Glif rengi (69,39,160)'a yakin (L1 < 150), 7-14 px kutulu, 22-70 piksellik
halka bilesenleri (bu kitapta simge 13x13: 7-12 px siniri yalniz 5 simge
buluyordu, olculdu); etrafindaki 30x30 pencerenin >= %30'u lila disk rengi
(240,238,247, L1 < 30). Donus [y_ust, x_sol, y_merkez, x_merkez]. Toplam 1005.

KANAL 2 -- BASILI SORU NUMARASI
-------------------------------
Bu kitapta numaralar SIYAH: koyu (max kanal < 110) ve notr (doygunluk < 40)
bilesenler, yalniz numara seridinde (sol x 69-90, sag x 373-395, y 95-900).
Ayni satirdaki rakam ve nokta bilesenleri tek numaraya birlestirilir; 12
px'ten uzun bilesenler atilir. Toplam 1740 (seritte satir basi metin de
sayilir; kutu araci yalniz <= 12 px genislikleri numara sayar).
...
KULLANIM
--------
    python backend/scripts/kitap/prg345_tarama.py
"""
...
HEDEF = CIKTI / "345_2025_paragraf_capa_taramasi.json"
KART = (589, 43, 1331, 1020)
BEKLENEN_BOYUT = (1920, 1080)
BEKLENEN_SAYFA = 368
# Icerik 3-355; 356-368 kitap sonu cevap anahtari (taranmaz).
ICERIK_SAYFA = 355
GLIF = np.array([69, 39, 160])
DISK = np.array([240, 238, 247])
NUMARA_SERIDI = {"L": (69, 90), "R": (373, 395)}
Y_ARALIK = (95, 900)
...
def kaynak_dizin() -> Path:
    ...
    adaylar = [
        ...
        if p.is_dir() and p.name.startswith("345 2025 Paragraf Sifir Risk")
    ]
    if len(adaylar) != 1:
...
def simgeler(a: np.ndarray) -> list[list[int]]:
    ...
    for i, sl in enumerate(ndimage.find_objects(lab)):
        ...
        if not (7 <= h <= 14 and 7 <= w <= 14 and 22 <= c <= 70):
            continue
...
def numaralar(a: np.ndarray) -> dict[str, list[list[int]]]:
    ...
    cm = (a.max(axis=2) < 110) & ((a.max(axis=2) - a.min(axis=2)) < 40)
...
def main() -> None:
    ...
    veri = {
        ...
        "arac": "scripts/kitap/prg345_tarama.py",
        "kart": list(KART),
