#!/usr/bin/env python
"""345 2025 Paragraf Sifir Risk: transkripsiyon harness'i (hazirla / topla / kapi).
...
---------------------------------------
1. Her kirpim tam bir kez okunmus olmali (1012 dosya, eksik/fazla yok).
...
4. Toplam soru sayisi 1012.
5. Ortak metin: 30 kayit, basliktaki kapsamin ilk sorusu == kutu dosyasindaki
   ilk soru, kapsam testin icinde, metin dolu (`ortak_kapisi`).

KULLANIM
--------
    python backend/scripts/kitap/prg345_metin_harness.py hazirla
    python backend/scripts/kitap/prg345_metin_harness.py topla
    python backend/scripts/kitap/prg345_metin_harness.py kapi
"""
...
KUTULAR = CIKTI / "345_2025_paragraf_kirpim_kutulari.json"
HEDEF = CIKTI / "345_2025_paragraf_metin.json"
KIRPIM = KOK / "backend" / "_p345_gecici" / "p_kirpim"
OKUMA_KOKU = Path(r"C:\Users\husey\VeraFilm\p_okuma_kirpim")
PARCA_DIZINI = Path(r"C:\Users\husey\VeraFilm\p_metin_parca")
NORMALIZASYON = CIKTI / "345_2025_paragraf_kusur_normalizasyonu.json"
BEKLENEN_SORU = 1012
BEKLENEN_ORTAK = 30
GRUP_SORU = 40
OLCEK = 2
...
def numarasi_ortulu() -> set[str]:
    ...
    (`prg345_kutu.numara_disk_ortulu`, kutuda `numara_disk_ortulu`).
    """
    return {
        _ad(k["birim"], k["soru"])
        for k in _kutular()
...
def topla() -> None:
    ...
    for i, s in enumerate(sorular):
        if s["dosya"] in duzeltme:
            ...
            sorular[i] = yeni
    # Kusur notu normalizasyonu (KIM hattindan kalan secenek; bu kitapta
    # kayit dosyasi YOK, adim bos gecer). Yalniz not metni BIREBIR
    # eslesirse bosaltilir; okunan metin degismez.
    dusur = {}
    ...
    ortak_yolu = PARCA_DIZINI / "ortak.jsonl"
    ortaklar = (
        [json.loads(x) for x in ortak_yolu.read_text("utf-8").splitlines() if x.strip()]
        if ortak_yolu.exists()
        else []
    )
    ortaklar.sort(key=lambda o: o["ortak"])
    HEDEF.write_text(
        json.dumps(
            {
                "kaynak": "345 2025 Paragraf Sifir Risk Soru Bankasi",
                "nereden": (
                    "Soru kirpimlarindan okundu (prg345_kirp.py, 2x Lanczos). Sorular "
                    "cozulmedi, cevap anahtari okuyucuya gosterilmedi."
                ),
                "parca_sayisi": len(parca),
                "soru_sayisi": len(sorular),
                "sorular": sorular,
                "ortak_metin_sayisi": len(ortaklar),
                "ortak_metinler": ortaklar,
            },
            ensure_ascii=True,
            indent=1,
        )
...
def ortak_kapisi(ortaklar: list[dict]) -> list[str]:
    """KAPI5: ortak metin sayisi, kutusu, kapsami (okuyucuya soylenmeyen yapidan).

    Basliktaki kapsamin ilk sorusu == kutu dosyasindaki ortak metnin hemen
    altindaki ilk soru; kapsamin tum sorulari ayni testte var; metin bos degil.
    """
    hata = []
    kutu = {
        o["ortak"]: o for o in json.loads(KUTULAR.read_text("ascii"))["ortak_metinler"]
    }
    test_soru: dict[str, set[int]] = {}
    for k in _kutular():
        test_soru.setdefault(k["birim"], set()).add(k["soru"])
    if len(ortaklar) != BEKLENEN_ORTAK:
        hata.append(f"KAPI5 ortak metin {len(ortaklar)} != {BEKLENEN_ORTAK}")
    for o in ortaklar:
        ad = o["ortak"]
        if ad not in kutu:
            hata.append(f"KAPI5 {ad}: kutusu yok")
            continue
        ilk = kutu[ad]["ilk_soru"]
        a, b = o["kapsam"]
        if a != ilk["soru"]:
            hata.append(f"KAPI5 {ad}: kapsam basi {a} != ilk soru {ilk['soru']}")
        if not (a < b and set(range(a, b + 1)) <= test_soru[ilk["birim"]]):
            hata.append(f"KAPI5 {ad}: kapsam {a}-{b} testte yok")
        if not str(o.get("metin", "")).strip():
            hata.append(f"KAPI5 {ad}: metin bos")
    return hata


def kapi(
    sorular: list[dict] | None = None, ortaklar: list[dict] | None = None
) -> list[str]:
    if sorular is None:
        veri = json.loads(HEDEF.read_text("ascii"))
        sorular = veri["sorular"]
        ortaklar = veri.get("ortak_metinler", [])
    hata = [] if ortaklar is None else ortak_kapisi(ortaklar)
