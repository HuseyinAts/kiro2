#!/usr/bin/env python
"""345 2025 AYT Matematik: soru kirpim kutularini uretir.

CAPA SECIMI -- KITABIN CEVAP SATIRI KARAR VERIR
-----------------------------------------------
Iki bagimsiz piksel kanali (`mat345ayt_tarama.py`): basili soru numarasi ve
...
- Kutu ustu: capadan yukari, okuyucu katmani beyazlatilmis sayfada, numara
  ...
  bant tavani (mat345ayt_tarama.bant_tavani).
...
- OSYM Tadinda sayfalarinin acik mavi dikey cercevesi beyazlatilir
  (mat345ayt_tarama.cerceve_maskesi).
...
KULLANIM
--------
    python backend/scripts/kitap/mat345ayt_kutu.py
    python backend/scripts/kitap/mat345ayt_kutu.py --yaz
"""
...
from mat345ayt_tarama import bant_tavani, cerceve_maskesi, kart, kaynak_dizin

KOK = Path(__file__).resolve().parents[3]
CIKTI = KOK / "veriseti" / "zkitap" / "cikti"
TARAMA = CIKTI / "345_2025_ayt_matematik_capa_taramasi.json"
ANAHTAR = CIKTI / "345_2025_ayt_matematik_cevap_anahtari.json"
HEDEF = CIKTI / "345_2025_ayt_matematik_kirpim_kutulari.json"
...
def kutulari_uret() -> dict[str, Any]:
                ...
                kutular.append(
                    {
                        ...
                        "numara_disk_ortulu": kaynak_kanal == "numara"
    ...
    return {
        ...
        "capa": "sutun basina: basili numara sayisi == cevap satiri girdi sayisi ise numara, degilse simge (345_2025_ayt_matematik_capa_taramasi.json)",
        "kural": (
            f"Kutu ustu: capadan yukari ilk {BOSLUK} satirlik bos bandin alt ucu - {UST_PAY} "
            "(tavan onceki capa / sayfanin bant tavani, mat345ayt_tarama.bant_tavani); "
