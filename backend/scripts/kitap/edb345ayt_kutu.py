#!/usr/bin/env python
"""345 2025 AYT Turk Edebiyati: soru ve ortak metin kirpim kutularini uretir.

CAPA SECIMI -- SUTUNUN SORU SAYISI KARAR VERIR
----------------------------------------------
Iki bagimsiz piksel kanali (`edb345ayt_tarama.py`): basili soru numarasi ve
FERNUS okuyucu simgesi. Bir sutunun capasi, sayisi o sutunun soru sayisina
(kitap sonu anahtarin girdileri, bant okumasiyla sayfaya/sutuna dagitilmis)
ESIT olan kanaldir; once numara (genisligi <= NUMARA_EN_GENIS; konu kutusundaki
genis mavi rakamlar capa degildir), o tutmazsa simge, o da tutmazsa iki
kanalin BIRLESIGI (ayni soruyu gosteren numara+simge BIRLESIK_Y icinde tek
capa); kanal sayisi fazlaysa en alttaki k tanesi (numara_alt / simge_alt).
Hicbiri tutmazsa sutuna kutu URETILMEZ (tahmin yok).
Olculen (612 soru sutunu): numara 611, simge 1; kutusuz sutun 0. Numarasi
okuyucu diski altinda kalan kutu 18 (DISK_NUMARA_EN_AZ).
...
- Kutu ustu: capadan yukari, okuyucu katmani beyazlatilmis sayfada, numara
  ...
  ya da sayfanin bant tavani (edb345ayt_tarama.bant_tavani).
- 'OSYM KOSESI' logosu (turuncu ~(245,135,25), >= 150 piksel) capanin
  LOGO_PENCERE px ustundeyse kutu ustu logonun ustu - 6'dir (61 kutu).
- Bant capadan UST_EN_COK px'ten daha yukarida kaliyorsa, capanin 25 px
  ustune kadar >= 3 satirlik kisa bos bant aranir (1 kutu).
- Konu kutusu kenari (ayrac_bandi_sonu) ve OSYM Tadinda mavi cercevesi
  (edb345ayt_tarama.cerceve_maskesi) KIM hattindaki gibi; bu kitapta ayrac
  bandi kaydirmasi 0.
- Kutu alti: ayni sutundaki sonraki kutunun ustu - 1; sutunun son sorusunda
  SAYFA_ALTI. Bu kitapta sayfa alti cevap seridi yok; notr (siyah) metin 612
  sutunun hepsinde y <= 879'da bitiyor, alttaki yapboz/kose susu renkli.
- Olcum (kapi degil, rapor): kutu ustunun +-3 satirinda notr koyu metin
  varsa kutu 'kesim_metne_degen_kutu' listesine girer (2; ikisi de OSYM
  KOSESI logosu, gozle tam).

ORTAK METIN
-----------
'7 - 8. sorulari asagidaki parcaya gore cevaplayiniz' basligi kirmizi
(~(190,43,48)) cerceveli; `ortak_basliklari` bir sutunda >= ORTAK_KOSU px
kirmizi yatay kosu ciftini (ORTAK_ARALIK) baslik sayar. Baslik sutunun ilk
sorusunun ustundeyse ortak metin kutusu = baslik ustu - UST_PAY .. ilk soru
kutusu ustu - 1. Olculen: 11 baslik, hepsi sutun basinda; soru kutusu
ICINDE baslik 0 (kapi). Ayni 11 sutun ikinci kanalla da (ilk kutusu
y > 175'te baslayan sutunlar) bulundu.

YATAY KURAL
-----------
Sutunlar arasi ara cizgi taramadan (cift sayfa x 369, tek sayfa x 372).
Cizgi olculemezse tek/cift varsayilani. Sayfanin renkli kenar seridi
disarida birakilir:
...
KULLANIM
--------
    python backend/scripts/kitap/edb345ayt_kutu.py
    python backend/scripts/kitap/edb345ayt_kutu.py --yaz
"""
...
from edb345ayt_tarama import bant_tavani, cerceve_maskesi, kart, kaynak_dizin

KOK = Path(__file__).resolve().parents[3]
CIKTI = KOK / "veriseti" / "zkitap" / "cikti"
TARAMA = CIKTI / "345_2025_ayt_edebiyat_capa_taramasi.json"
ANAHTAR = CIKTI / "345_2025_ayt_edebiyat_cevap_anahtari.json"
HEDEF = CIKTI / "345_2025_ayt_edebiyat_kirpim_kutulari.json"
...
def ortak_basliklari(a: np.ndarray, x0: int, x1: int) -> list[list[int]]:
    """Ortak metin basligi kutulari ('7 - 8. sorulari asagidaki parcaya gore
    cevaplayiniz'): kirmizi (~(190,43,48)) cerceve. Sutunda >= ORTAK_KOSU px
    kesintisiz kirmizi yatay kosu bir kenardir; ust+alt kenar cifti
    (ORTAK_ARALIK icinde) bir basliktir. Donus: [[y_ust, y_alt], ...]."""
    b = a[:, x0:x1].astype(np.int16)
    r = (b[..., 0] >= 150) & (b[..., 0] <= 225) & (b[..., 1] < 80) & (b[..., 2] < 90)
    satir = []
    for y in range(100, SAYFA_ALTI):
        if r[y].sum() < ORTAK_KOSU:
            continue
        en = n = 0
        for v in r[y]:
            n = n + 1 if v else 0
    ...
    i = 0
    while i + 1 < len(kenar):
        if ORTAK_ARALIK[0] <= kenar[i + 1][0] - kenar[i][1] <= ORTAK_ARALIK[1]:
            out.append([kenar[i][0], kenar[i + 1][1]])
            i += 2
        else:
            i += 1
    return out
...
def kutulari_uret() -> dict[str, Any]:
    ...
    for d in sorted({k[0] for k in sutunlar}):
        ...
        for t in "LR":
            ...
            for h in ortak_basliklari(a, x0, x1):
                if h[1] < ustler[0]:
                    ortak.append(
                        {
                            "ortak": f"EDB345AYT-O{d:03d}{t}",
                            "dosya": d,
                            "sutun": t,
                            "kutu": [x0, h[0] - UST_PAY, x1, ustler[0] - 1],
                            "baslik": h,
                            "ilk_soru": {
                                "birim": qs[0]["birim"],
                                "soru": qs[0]["soru"],
    ...
    return {
        ...
        "capa": "sutun basina: basili numara sayisi == sutunun soru sayisi (kitap sonu anahtar) ise numara, degilse simge (345_2025_ayt_edebiyat_capa_taramasi.json)",
        "kural": (
            f"Kutu ustu: capadan yukari ilk {BOSLUK} satirlik bos bandin alt ucu - {UST_PAY} "
            "(tavan onceki capa / sayfanin bant tavani, edb345ayt_tarama.bant_tavani); "
        ...
        "kutusuz": kutusuz,
        "ortak_metin_kurali": (
            "Kirmizi cerceveli ortak metin basligi (ortak_basliklari) sutunun ilk sorusunun "
            "ustundeyse ortak metin kutusu: baslik ustu - UST_PAY .. ilk soru kutusu ustu - 1."
        ),
