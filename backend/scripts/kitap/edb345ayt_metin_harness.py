#!/usr/bin/env python
"""345 2025 AYT Turk Edebiyati: transkripsiyon harness'i (hazirla / topla / kapi).
...
---------------------------------------
1. Her kirpim tam bir kez okunmus olmali (1384 dosya, eksik/fazla yok).
...
4. Toplam soru sayisi 1384.
5. Ortak metin: 11 kayit, basliktaki kapsamin ilk sorusu == kutu dosyasindaki
   ilk soru, kapsam testin icinde, metin dolu (`ortak_kapisi`).

KULLANIM
--------
    python backend/scripts/kitap/edb345ayt_metin_harness.py hazirla
    python backend/scripts/kitap/edb345ayt_metin_harness.py topla
    python backend/scripts/kitap/edb345ayt_metin_harness.py kapi
"""
...
HEDEF = CIKTI / "345_2025_ayt_edebiyat_metin.json"
KIRPIM = KOK / "backend" / "_e345_gecici" / "e_kirpim"
OKUMA_KOKU = Path(r"C:\Users\husey\VeraFilm\e_okuma_kirpim")
PARCA_DIZINI = Path(r"C:\Users\husey\VeraFilm\e_metin_parca")
NORMALIZASYON = CIKTI / "345_2025_ayt_edebiyat_kusur_normalizasyonu.json"
BEKLENEN_SORU = 1384
BEKLENEN_ORTAK = 11
GRUP_SORU = 45
OLCEK = 2
...
def numarasi_ortulu() -> set[str]:
    ...
    (`edb345ayt_kutu.numara_disk_ortulu`, kutuda `numara_disk_ortulu`).
    """
    return {
        _ad(k["birim"], k["soru"])
        for k in _kutular()
        ...
        json.dumps(
            {
                ...
                "nereden": (
                    "Soru kirpimlarindan okundu (edb345ayt_kirp.py, 2x Lanczos). Sorular "
