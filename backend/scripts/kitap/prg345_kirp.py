#!/usr/bin/env python
"""345 2025 Paragraf Sifir Risk: kirpim kutularindan soru ve ortak metin gorselleri uretir.

Gorseller git'e girmez; her kutu `345_2025_paragraf_kirpim_kutulari.json`
...
NOTR_DOYGUNLUK) varsa rapor; renkli sayfa susu (OSYM KOSESI kirmizi
cercevesi) sayilmaz.

KULLANIM
--------
    python backend/scripts/kitap/prg345_kirp.py
    python backend/scripts/kitap/prg345_kirp.py --ornek 40
    python backend/scripts/kitap/prg345_kirp.py --cikti <dizin> --rapor <json>
"""
...
from prg345_tarama import cerceve_maskesi, kart, kaynak_dizin
...
def main() -> None:
    ...
    ap.add_argument(
        "--cikti", default=str(KOK / "backend" / "_p345_gecici" / "p_kirpim")
    )
    ap.add_argument(
        "--rapor",
        default=str(CIKTI / "345_2025_paragraf_ortme_olcumu.json"),
    ...
    for p in sorted(sayfada):
        ...
        # Notr (siyah/gri) murekkep: OSYM KOSESI'nin kirmizi cerceve kenari
        # (~(236,28,37)) kutu kenarinda durur, metin degildir (37 kutu olculdu).
    ...
    # Ortak metin kutulari (koyu cerceveli baslik / ELLE_BASLIK; prg345_kutu).
    ...
    if not args.ornek:
        for o in ortaklar:
            ...
            a[okuyucu_maskesi(a, s)] = 255
            a[cerceve_maskesi(a, o["dosya"])] = 255
            x0, y0, x1, y1 = o["kutu"]
    ...
    if args.rapor:
        rapor = {
            ...
            "arac": "scripts/kitap/prg345_kirp.py",
            "ne_olculdu": (
