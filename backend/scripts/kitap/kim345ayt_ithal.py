"""345 2025 AYT Kimya Soru Bankasi -- PASIF ithal.

KAYNAK VE TAVANI
----------------
FERNUS okuyucusunun 1920x1080 ekran goruntuleri; sayfa karti (589, 43) -
(1331, 1020) = 742x977 (345 matematik kitaplariyla ayni, olculdu). 336 PNG; basili
sayfa numarasi = dosya numarasi. '345 2025 Ayt Kimya' yakalamasi isleniyor.
'345 2024 Ayt Kimya' klasoru onceki baskidir (yaklasik 13 soru degismis);
bu ithalde kullanilmadi, yalniz eski hat celiskilerinde ayni sayfanin
2024 seridiyle karsilastirma icin acildi.

BIRIM = TEST
------------
Her soru sayfasinin altinda sutun basina cevap satiri var (konu anlatimi
sutunlarinda yok); numara kitap boyunca 160 kez 1'e donuyor ve her donus
kitabin baslik bandindaki bir testle basliyor: 100 Kazanim Odakli (konu
duzeyi), 48 OSYM Tadinda ve 12 Orijinal Sorular (unite duzeyi). 1304 soru.

CEVAP KAYNAGI: SAYFA ALTI CEVAP SATIRI
--------------------------------------
Anahtar kitabin KENDI basili satirlarindan iki bagimsiz gorsel okumayla
(A 5x sirali, B 7x ters sirali) okundu, 1304/1304 ayni; ucuncu kanal piksel
en-yakin-komsu (LOO 1253/1282), uyumsuzlarin ve okuyucu tereddutlerinin
hepsi goz ile. Hicbir soru cozulmedi.

KONU AGACI
----------
Kitabin kendi icindekiler sayfalari (dosya 3-4): 12 unite + 50 konu (0049,
KIM kokunun altinda KIM-345A25 oneki). Kazanim Odakli testler KONU
dugumune, OSYM Tadinda ve Orijinal testler UNITE dugumune baglanir
(`konu_eslesme_duzeyi`).

CIKMIS SORULAR
--------------
'OSYM KOSESI' kutusundaki basili etiket (ornek 'AYT - 2018') transkripsiyonda
...
sik kaybolur. Belirsiz iyon yuku isareti `?` yazildi (or. `Cu^(2?)`) ve
`kaynak_kusuru`na not dusuldu; bu sorular `kaynak_kusuru` bayragi tasir.
Tum iyon yuklu sorular (178) yuk isareti hakemliginden gecti.
...
---------------------------------------
345_2025_ayt_kimya_mukerrer_adaylari.json: govde kelime kumesi Jaccard
>= 0.75 VE bes sikkin >= 3'u birebir olan GUCLU adaylar `mukerrer_aday`
bayragi alir. Eski hattaki cevap celiskileri 0051 ile basili cevaba cekildi.

IKINCI OKUMA
------------
On kayitli orneklem (210) ust siniri %3 esigini asti (5/210, %5.47) ve bes
hatanin besi de iyon yuku isareti tabakasindaydi; o tabaka (124 soru) ayni
kuralla ikinci kez okundu, 68 soruda isaret duzeltildi. Duzeltmeler ithal
ONCESI yapildigi icin id sabitlemesi gerekmez.

BILINEN BORC
------------
  * Yuk isareti tabakasi disindaki sorular TAM ikinci okunmadi; orneklemde
    tabaka disi esasli hata 0.
  * Cozumler kitapta soru sayfasinda YOK; `explanation` bos.

Detay: veriseti/zkitap/cikti/KIM_345_AYT_YONTEM.md
"""
...
ONEK_DOSYA = f"{CIKTI}/345_2025_ayt_kimya"
...
KAYNAK_ADI = "345 2025 AYT Kimya Soru Bankasi"
ONEK = KAYNAK_KAYITLARI[KAYNAK_ADI]["onek"]
KIM_KOK_KODU = "KIM"
KOD_ONEKI = "KIM-345A25"
SINIF_DUZEYI = 12
ITHAL_ARACI = "scripts/kitap/kim345ayt_ithal.py"
CROP_ONEK = "KIM345_AYT"
TELIF_NOTU = (
...
URETIM_NOTU = (
    ...
    "alti cevap satirindan iki bagimsiz gorsel okumayla alindi (1304/1304 ayni; "
    "piksel en-yakin-komsu ucuncu kanal; uyumsuz/tereddutlu girdiler goz ile). "
    "Sorular cozulmedi. Kirpim kutulari basili soru numarasindan ya da okuyucu "
    "simgesinden turetildi. Transkripsiyon 21 grupta ayri okuyucularla kirpimdan "
    "yapildi; anahtar gosterilmedi; iyon yuku isaretleri ayrica hakemlendi. "
    "Detay: veriseti/zkitap/cikti/KIM_345_AYT_YONTEM.md"
)
ANAHTAR_DOGRULAMASI = (
    "sayfa_alti_satir_iki_okuma_1304_1304_ayni__piksel_nn_loo_1253_1282__"
    "uyumsuz_29_tereddut_211_kapsam_disi_9_yarim_satir_goz__"
    "numara_1_ya_da_onceki_arti_1_160_test__"
...
# simge_alt kanallarinda numara okuyucu diski altinda kalabilir (kim345ayt_kutu).
...
def kayit_uret(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pipeline_metadata": {
            ...
            "yuk_isareti_hakemligi": r.get("hakem_notu"),
            "kirpim_kutusu": kutu,
            ...
            "metin_kaynagi": "soru_kirpimi_gorsel_okuma_21_grup_ve_duzeltme",
...
VALUES (%(id)s, %(bloom_level)s, %(bloom_category)s, 'AYT', 'KIMYA', %(grade_level)s,
...
def _duzey_kontrol(k: dict[str, Any]) -> list[str]:
    """Konu kodu (KIM-345A25-Unn-kk) konu, unite kodu (KIM-345A25-Unn) unite duzeyidir."""
...
def _konu_id_ata(
    ...
    if not kok:
        return f"{KIM_KOK_KODU} kok konusu yok"
    ...
    if eksik:
        return (
            ...
            "0049_kim345ayt_konu_agaci kosmadi mi? Kok dugume dusurup "
