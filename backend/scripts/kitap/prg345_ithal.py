"""345 2025 Paragraf Sifir Risk Soru Bankasi -- PASIF ithal.

KAYNAK VE TAVANI
----------------
FERNUS okuyucusunun 1920x1080 ekran goruntuleri; sayfa karti (589, 43) -
(1331, 1020) = 742x977 (diger 345 kitaplariyla ayni, olculdu). 368 PNG;
icerik 3-355, kitap sonu cevap anahtari 356-368. Basili sayfa numarasi =
dosya numarasi. Tek yakalama (kurtarma kanali yok).

BIRIM = TEST
------------
7 bolum ('Sifir Risk 01-07'), 85 test, 1012 soru: bolum 1-3'te 9'ar test
x 20; bolum 4'te 3 Kondisyon (8'er), 7 Odaklanma (20'ser) ve OSYM Cikmis
Sorular Ozel Denemesi (30); bolum 5'te 15 temali test; bolum 6'da 22 'OSYM
Tadinda Sorular'; bolum 7'de 10 test. '6. NUANS TESTI' (s340-343) acik
uclu, anahtarda yok -> kapsam disi.

CEVAP KAYNAGI: KITAP SONU CEVAP ANAHTARI
---------------------------------------
Kitabin KENDI basili anahtari (dosya 356-368). Iki bagimsiz gorsel okuma
(A 3x dilimli sirali, B 3x tam sayfa ters sirali): 85/85 test, 1012/1012
girdi, harf farki 0; '?' tasiyan 34 girdi 10x gozle. Tablo sayfalarinda
(365-368) harf glifi en-yakin-komsu ucuncu kanal LOO 441/441. Hicbir soru
cozulmedi. Soru -> sayfa/sutun konumu testin sayfalarindan (bant okumasi)
ve sutun basina basili numara / simge sayisindan.

KONU AGACI
----------
Kitabin bolumleri (0055, TUR kokunun altinda TUR-345P25-B01..B07). Her test
bolum dugumune baglanir (`konu_eslesme_duzeyi` = 'bolum').

ORTAK METIN
-----------
'7 - 8. sorulari asagidaki parcaya gore cevaplayiniz' gibi koyu cerceveli
baslikli 29 parca + 1 cercevesiz yonerge (s170L '18 - 20. sorularda ...')
ayri okundu; kapsamdaki her sorunun govdesinin basina bos satirla eklenir
(`ortak_metin`, `ortak_metin_govdede` bayragi). Gorsel yalniz soru kirpimidir.

CIKMIS SORULAR
--------------
Soru altindaki basili kaynak etiketi (ornek '2021 - ALES', '2025 - MSU')
`etiket` alaninda; yalniz bu sorularda `osym_year` ve
`osym_format_compliant` doldurulur.

BULANIK METIN (TASARIM)
-----------------------
'Algisal Butunluk Testi' parcalari BILEREK bulaniklastirilmis; govde
'[bulanik metin]' + soru koku, `bulanik_metin_tasarim` bayragi.

ORTME: ITHAL EDILIR, ISARETLENIR
--------------------------------
Okuyucu diskinin kenar halkasinda kitap murekkebi olculen sorular
`okuyucu_diski_ortme` bayragi tasir. Numarasi gorunmeyen sorular
`numara_ortulu` bayragi tasir.

MUKERRER / ESKI HAT
-------------------
345_2025_paragraf_mukerrer_adaylari.json: GUCLU adaylar (govde Jaccard >=
0.75 VE >= 3 sik birebir) `mukerrer_aday`; bu kitabin 8 eski hat satiriyla
ayni sayfadaki eslesme `eski_hat_ikizi` (cevaplarin 8'i basili anahtarla
ayni). Eski satirlara DOKUNULMAZ.

IKINCI OKUMA
------------
On kayitli orneklem (200) ust siniri %3.11 > %3 -> on kayit kurali geregi
TAM ikinci okuma yapildi (1012 soru + 30 ortak metin); her kelime farki
kirpimdan 3x kesitle gozle hukme baglandi, ilk okuma hatalari duzeltildi.

Detay: veriseti/zkitap/cikti/PRG_345_YONTEM.md
"""
...
ONEK_DOSYA = f"{CIKTI}/345_2025_paragraf"
...
KAYNAK_ADI = "345 2025 Paragraf Sifir Risk Soru Bankasi"
ONEK = KAYNAK_KAYITLARI[KAYNAK_ADI]["onek"]
TUR_KOK_KODU = "TUR"
KOD_ONEKI = "TUR-345P25"
SINIF_DUZEYI = 12  # mevcut TYT satirlarinin ev sozlesmesi (mat345tyt / biyo345tyt)
...
URETIM_NOTU = (
    "FERNUS okuyucu ekran goruntusunden okuma hatti. Cevaplar KITABIN kitap "
    "sonu cevap anahtarindan (dosya 356-368) iki bagimsiz gorsel okumayla alindi "
    "(1012 girdi, 0 harf farki; tablo sayfalarinda glif en-yakin-komsu ucuncu kanal "
    "LOO 441/441; tereddutlu 34 girdi goz ile). Sorular cozulmedi. Kirpim kutulari "
    "basili soru numarasindan ya da okuyucu simgesinden turetildi. Transkripsiyon "
    "25 grupta ayri okuyucularla kirpimdan yapildi, on kayitli orneklem kararinca TAM "
    "ikinci okumayla karsilastirildi; anahtar gosterilmedi. Ortak metinli sorularda "
    "parca soru govdesinin basina eklendi. "
    "Detay: veriseti/zkitap/cikti/PRG_345_YONTEM.md"
)
ANAHTAR_DOGRULAMASI = (
    "kitap_sonu_anahtar_iki_okuma_1012_girdi_0_harf_farki__"
    "tablo_sayfalari_glif_nn_loo_441_441__tereddut_34_goz__"
    "test_sirasi_85_85_bant_okumasiyla_ayni__"
    "sutun_basina_soru_sayisi_85_85_test__"
    "basili_numara_esittir_test_ici_sira_numarasi_gorunmeyenler_haric"
)
CEVAP_KAYNAGI = "kitap_sonu_cevap_anahtari"
ORTAK_AYRAC = "\n\n"
BEKLENEN_ORTAK = 30
CEVAP_KANALLARI = ("iki_okuma", "iki_okuma(biri_tereddutlu)+goz")
# Kutu capasi basili numaradan alindiysa numara gorunur; simge / birlesik
# kanallarinda numara okuyucu diski altinda ya da basilmamis olabilir.
NUMARA_KANALLARI = ("numara", "numara_alt")
...
BULANIK = "[bulan\u0131k metin]"
# '2021 - ALES' / '2025 - MS<U+00DC>' / 'TYT - 2021': sinav adi + yil.
_ETIKET = re.compile(
    r"(TYT|AYT|YGS|LYS|MS\u00dc|MSU|KPSS|ALES|DGS|\u00d6SYM|OSYM)|((?:19|20)\d\d)"
)


def etiket_ayristir(etiket: str | None) -> tuple[str | None, int | None]:
    """'2021 - ALES' -> ('ALES', 2021). Etiket yoksa ya da bicim disiysa (None, None)."""
    if not etiket:
        return None, None
    sinav = yil = None
...
def _bayraklar(r: dict[str, Any], sec: dict[str, str]) -> list[str]:
    ...
    if r.get("eski_hat"):
        b.append("eski_hat_ikizi")
    if r.get("ortak_metin"):
        b.append("ortak_metin_govdede")
    if r.get("sinav_yili"):
        b.append("cikmis_soru")
    if BULANIK in r["govde"]:
        b.append("bulanik_metin_tasarim")
...
def kayit_uret(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pipeline_metadata": {
            "kaynak": ONEK.lower(),
            "konu_kodu": r["konu_kodu"],
            "konu_eslesme_duzeyi": "bolum",
            "konu_kaynagi": "bolum_ayraci_sayfalari_ve_test_baslik_bandi",
            ...
            "eski_hat_ikizi": r.get("eski_hat"),
            "ortak_metin": r.get("ortak_metin"),
            "ortak_metin_kapsami": r.get("ortak_metin_kapsami"),
            "kirpim_kutusu": kutu,
            ...
            "metin_kaynagi": "soru_kirpimi_gorsel_okuma_25_grup_tam_ikinci_okuma_ve_duzeltme",
...
def satirlari_bagla(
    ...
    *,
    ...
    AYRICA dogrulanir ki kutu-capa hatasi sessizce kaymasin (numarasi
    gorunmeyen, capasi simgeden alinmis ya da diskin ortugu kutu haric).
    """
    ...
    eski = {
        a["dosya"]: {
            k: a[k] for k in ("db_id", "jaccard", "ayni_sik_sayisi", "cevap_ayni")
        }
        for a in mukerrer.get("eski_hat_satirlari", [])
    }
    # Ortak metin: kapsamdaki her sorunun govdesinin basina (kutu dosyasindaki
    # ilk_soru testinde, kapsam[0]..kapsam[1]).
    ortak_kutu = {o["ortak"]: o for o in kutular["ortak_metinler"]}
    ortak: dict[tuple[str, int], dict] = {}
    for o in veri.get("ortak_metinler", []):
        ilk = ortak_kutu[o["ortak"]]["ilk_soru"]
        if o["kapsam"][0] != ilk["soru"]:
            raise ValueError(f"{o['ortak']}: kapsam basi ilk soruya esit degil")
        for n in range(o["kapsam"][0], o["kapsam"][1] + 1):
            ortak[(ilk["birim"], n)] = o
    satir = []
    for okunan in veri["sorular"]:
        kod, sira = okunan["dosya"].rsplit("_", 1)
        sira_i = int(sira)
        om = ortak.get((kod, sira_i))
        s = (
            okunan
            if om is None
            else {**okunan, "govde": om["metin"] + ORTAK_AYRAC + okunan["govde"]}
        )
        k = kutu[(kod, sira_i)]
        if s["basili_no"] != sira_i and not (
            s["basili_no"] is None
            and (k["capa_kanali"] not in NUMARA_KANALLARI or k["numara_disk_ortulu"])
        ):
        ...
        satir.append(
            {
                **s,
                ...
                # Sikkin tamami cizim: "(gorsel sik)" (ya da "(gorsel sik) <cumle>").
                "sikler_gorsel": all(
                ...
                "eski_hat": eski.get(s["dosya"]),
                "sinav": sinav,
                "sinav_yili": yil,
                "ortak_metin": om["ortak"] if om else None,
                "ortak_metin_kapsami": om["kapsam"] if om else None,
            }
        )
    return satir


def kitap_ici_tekrarlar(kayitlar: list[dict[str, Any]]) -> int:
    """Kitap AYNI soruyu iki FARKLI testte basmis: iki soru da ithal edilir.

    Olculdu: PRG345-T022_20 (s146, OSYM Tadinda) ile PRG345-T038_13 (OSYM
    Cikmis Sorular Ozel Denemesi) ayni cikmis soru; iki ayri kirpim, iki
    bagimsiz okumada metin ve siklar birebir, iki testin anahtari ayni harf.
    soru_hash DEGISMEZ (aktif satirlarda benzersizlik kisiti; bu ithal pasif);
    id = uuid5(hash | kitap_ici_tekrar | kirpim adi) ve kayitlar birbirini
    `kitap_ici_tekrar_ile` alaninda gosterir. Yalniz uyelerin HEPSI farkli
    testteyse ayristirilir; ayni testte ayni hash gercek bir cift okumadir ve
    on kontrol 'ayni hash iki kez' ile durur.
    """
    ...
    for h, g in grup.items():
        birimler = [k["pipeline_metadata"]["birim_kodu"] for k in g]
        if len(g) < 2 or len(set(birimler)) != len(g):
            continue
        if any(k["pipeline_metadata"].get("sekil_ikizi_ile") for k in g):
            continue
        n += 1
        adlar = [k["pipeline_metadata"]["kaynak_gorseli"] for k in g]
        for k in g:
            ad = k["pipeline_metadata"]["kaynak_gorseli"]
            k["id"] = str(uuid.uuid5(uuid.NAMESPACE_OID, f"{h}|kitap_ici_tekrar|{ad}"))
            k["pipeline_metadata"]["kitap_ici_tekrar_ile"] = [
                a for a in adlar if a != ad
            ]
            k["pipeline_metadata"]["bayraklar"].append("kitap_ici_tekrar")
    return n
...
def _yapisal_kapilar(
    ...
    if len(veri.get("ortak_metinler", [])) != BEKLENEN_ORTAK:
        hata.append(
            f"ortak metin {len(veri.get('ortak_metinler', []))} != {BEKLENEN_ORTAK}"
        )
    if len(kutular.get("ortak_metinler", [])) != BEKLENEN_ORTAK:
        hata.append(
            f"ortak metin kutusu {len(kutular.get('ortak_metinler', []))} != {BEKLENEN_ORTAK}"
        )
...
def _duzey_kontrol(k: dict[str, Any]) -> list[str]:
    """Bolum kodu TUR-345P25-Bnn (tek duzey)."""
    if not re.fullmatch(rf"{KOD_ONEKI}-B0[1-7]", k["konu_kodu"] or ""):
        return [f"{k['id']}: konu kodu {k['konu_kodu']!r} bolum kodu degil"]
    if k["pipeline_metadata"]["konu_eslesme_duzeyi"] != "bolum":
        return [f"{k['id']}: eslesme duzeyi 'bolum' degil"]
    return []
...
def _on_kontrol(kayitlar: list[dict[str, Any]]) -> list[str]:
    ...
    for k in kayitlar:
        ...
        if pm["soru_no_basili"] != pm["birim_ici_sira"] and not (
            ...
            and (
                pm["kirpim_capa_kanali"] not in NUMARA_KANALLARI
...
def _konu_id_ata(
    ...
    if eksik:
        return (
            ...
            "0055_prg345_konu_agaci kosmadi mi? Kok dugume dusurup "
...
def _ozet(kayitlar: list[dict[str, Any]]) -> None:
    ...
    print(
        "test turu (ilk 8) :",
        dict(
            Counter(k["pipeline_metadata"]["test_turu"] for k in kayitlar).most_common(
                8
            )
        ),
    )
...
def _hazirla(yollar: dict[str, Path]) -> list[dict[str, Any]] | None:
    ...
    if yapisal:
        ...
        return None
    print(
        f"yapisal kapi: {BEKLENEN_SORU} soru + {BEKLENEN_TEST} test + "
        f"{BEKLENEN_BOLUM} bolum + "
    ...
    tekrar = kitap_ici_tekrarlar(kayitlar)
    if tekrar:
        print(f"kitap ici tekrar: {tekrar} grup (ayni soru iki farkli testte basili)")
