#!/usr/bin/env python
"""345 2025 AYT Kimya: capa taramasi (okuyucu simgesi + basili soru numarasi + ara cizgi).
...
(`kim345ayt_kutu.py`). Bu script yalniz olcer, karar vermez.
...
Bu kitapta 553 soru sutununun 25'inde capa yalniz bu kanaldan alindi.
...
KULLANIM
--------
    python backend/scripts/kitap/kim345ayt_tarama.py
"""
...
HEDEF = CIKTI / "345_2025_ayt_kimya_capa_taramasi.json"
...
def kaynak_dizin() -> Path:
    ...
    adaylar = [
        ...
        if p.is_dir() and p.name.startswith("345 2025 Ayt Kimya Soru Bankas")
    ]
    if len(adaylar) != 1:
...
def main() -> None:
    ...
    veri = {
        ...
        "arac": "scripts/kitap/kim345ayt_tarama.py",
        "kart": list(KART),
