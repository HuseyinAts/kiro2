"""345 2025 AYT Turk Edebiyati Soru Bankasi -- PASIF ithal.

KAYNAK VE TAVANI
----------------
FERNUS okuyucusunun 1920x1080 ekran goruntuleri; sayfa karti (589, 43) -
(1331, 1020) = 742x977 (345 matematik/kimya kitaplariyla ayni, olculdu). 348 PNG;
329-348 s328'in piksel-ozdes kopyasi (okuyucu takilmis), icerik 1-328. Basili
sayfa numarasi = dosya numarasi. Bu kitabin tek yakalamasi var (kurtarma kanali yok).

BIRIM = TEST
------------
Her sayfanin ust bandi test turunu ve sirasini basar (devam sayfalari tekrarlar).
148 test: 46 Kazanim Odakli + 58 OSYM Tadinda (konu duzeyi), 26 Karma + 8
Orijinal + 10 Genel Bakis (unite duzeyi; Genel Bakis = unite 10). 1384 soru.

CEVAP KAYNAGI: KITAP SONU CEVAP ANAHTARI
---------------------------------------
Kitabin KENDI basili anahtari (dosya 321-328, konu basina satirlar). Iki
bagimsiz gorsel okuma (A 3x dilimli sirali, B 2x tam sayfa ters sirali);
1384 girdi, harf farki 1 (goz + glif NN ile B). Ucuncu kanal harf glifi
en-yakin-komsu (LOO 1325/1325). Hicbir soru cozulmedi. Soru -> sayfa/sutun
konumu testin sayfalarindan (bant okumasi) ve sutun basina basili numara
sayisindan.

KONU AGACI
----------
Kitabin kendi icindekiler sayfalari (dosya 3-4): 10 unite + 46 konu (0053,
EDB kokunun altinda EDB-345A25 oneki). Kazanim Odakli ve OSYM Tadinda testler
KONU dugumune, Karma / Orijinal / Genel Bakis testleri UNITE dugumune baglanir
(`konu_eslesme_duzeyi`).

ORTAK METIN
-----------
'7 - 8. sorulari asagidaki parcaya gore cevaplayiniz' gibi kirmizi cerceveli
baslikli 11 parca (edb345ayt_kutu.ortak_basliklari) ayri okundu; kapsamdaki
her sorunun govdesinin basina bos satirla eklenir (`ortak_metin`,
`ortak_metin_govdede` bayragi). Gorsel yalniz soru kirpimidir.

CIKMIS SORULAR
--------------
'OSYM KOSESI' kutusundaki basili etiket (ornek 'AYT - 2024') transkripsiyonda
`etiket` alaninda; yalniz bu sorularda `osym_year` ve `osym_format_compliant`
doldurulur.
...
kalan sorular `numara_ortulu` bayragi tasir.

MUKERRER ADAYLARI ISARETLENIR, SILINMEZ
---------------------------------------
345_2025_ayt_edebiyat_mukerrer_adaylari.json: govde kelime kumesi Jaccard
>= 0.75 VE bes sikkin >= 3'u birebir olan GUCLU adaylar `mukerrer_aday`
bayragi alir. DB'de bu kitabin eski satiri yok.

IKINCI OKUMA
------------
On kayitli orneklem (224) ust siniri %1.63 (0 esasli hata); tam ikinci okuma
gerekmedi. Orneklemdeki iki ek hatasi (anlam ayni) ithal oncesi duzeltildi.

BILINEN BORC
------------
  * Cozumler kitapta soru sayfasinda YOK; `explanation` bos.
  * Kaynak cozunurlugu dusuk: `rn`~`m`, `t`~`l` birlesmeleri okuyucu kuraliyla
    gercek kelimeye cozuldu; iki adayli ozel adlar `kaynak_kusuru` notu tasir.

Detay: veriseti/zkitap/cikti/EDB_345_AYT_YONTEM.md
"""
...
ONEK_DOSYA = f"{CIKTI}/345_2025_ayt_edebiyat"
...
URETIM_NOTU = (
    ...
    "sonu cevap anahtarindan (dosya 321-328) iki bagimsiz gorsel okumayla alindi "
    "(1384 girdi, 1 harf farki; glif en-yakin-komsu ucuncu kanal LOO 1325/1325; "
    "uyumsuz/tereddutlu girdiler goz ile). Sorular cozulmedi. Kirpim kutulari "
    "basili soru numarasindan ya da okuyucu simgesinden turetildi. Transkripsiyon "
    "28 grupta ayri okuyucularla kirpimdan yapildi; anahtar gosterilmedi. Ortak "
    "metinli sorularda (kirmizi baslikli parca) metin soru govdesinin basina eklendi. "
    "Detay: veriseti/zkitap/cikti/EDB_345_AYT_YONTEM.md"
)
ANAHTAR_DOGRULAMASI = (
    "kitap_sonu_anahtar_iki_okuma_1384_girdi_1_harf_farki_goz__"
    "glif_nn_loo_1325_1325__tereddut_59_goz__"
    "test_sirasi_148_148_bant_okumasiyla_ayni__"
    "sutun_basina_numara_sayisi_147_148_test_1_sutun_simge__"
...
CEVAP_KANALLARI = (
    ...
    "iki_okuma(uyusmaz)+piksel+goz",
)
# Kutu capasi basili numaradan alindiysa numara gorunur; simge / birlesik /
# simge_alt kanallarinda numara okuyucu diski altinda kalabilir (edb345ayt_kutu).
...
# 'AYT - 2018' / 'LYS-2 - 2014' / 'YGS - 2017': sinav adi + yil (U+00DC = U-umlaut).
# Bu kitapta 2010-2017 cikmislari sinav adi yerine 'OSYM - 2015' basar (U+00D6 =
# O-umlaut): sinav 'OSYM' (hangi oturum oldugu basili degil, tahmin edilmez).
_ETIKET = re.compile(
...
def kayit_uret(r: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pipeline_metadata": {
            ...
            "metin_kaynagi": "soru_kirpimi_gorsel_okuma_28_grup_ve_duzeltme",
        ...
        satir.append(
            {
                **s,
                ...
                "konu_kodu": t["konu"],
                "konu_eslesme_duzeyi": t["eslesme_duzeyi"],
                "cevap": c["cevap"],
...
def _duzey_kontrol(k: dict[str, Any]) -> list[str]:
    """Konu kodu (EDB-345A25-Unn-kk) konu, unite kodu (EDB-345A25-Unn) unite duzeyidir."""
    duzey = "konu" if (k["konu_kodu"] or "").count("-") == 3 else "unite"
    beyan = k["pipeline_metadata"]["konu_eslesme_duzeyi"]
    if beyan != duzey:
        return [f"{k['id']}: eslesme duzeyi {beyan!r} kodla ({duzey}) tutarsiz"]
    return []
...
def _konu_id_ata(
    ...
    if eksik:
        return (
            ...
            "0053_edb345ayt_konu_agaci kosmadi mi? Kok dugume dusurup "
