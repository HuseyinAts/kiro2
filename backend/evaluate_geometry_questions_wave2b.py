"""
Wave 2B Quality Evaluation for Phase 3 Geometry Questions

Evaluates the 5 generated geometry questions with Wave 2B quality system.
Target: 0.85+ quality score (same as Phase 1 & 2)
...
async def load_geometry_questions():
    """Load the 5 generated geometry questions"""
    ...
    try:
        ...
        print(
            "[INFO] Please run demo_geometry_generation.py first to generate questions"
        )
        return []
...
async def evaluate_questions():
    """Evaluate geometry questions with Wave 2B"""

    print("\n" + "=" * 70)
    print("WAVE 2B QUALITY EVALUATION - PHASE 3 GEOMETRY QUESTIONS")
    print("=" * 70 + "\n")

    # Load questions
    questions = await load_geometry_questions()
    ...
    for i, question in enumerate(questions, 1):
        ...
        print(f"Has Geometry: {'Yes' if visual_content else 'No'}")

        if visual_content:
            geometry_info = visual_content.get("metadata", {})
            print(f"Geometry Type: {geometry_info.get('geometry_type', 'Unknown')}")
            print(f"Shape Subtype: {geometry_info.get('shape_subtype', 'Unknown')}")
            print(f"Dimensions: {geometry_info.get('dimensions', {})}")
            ...
            results.append(
                {
                    "question_id": i,
                    "test_case": test_case,
                    "geometry_type": geometry_info.get("geometry_type")
                    if visual_content
                    else None,
                    "shape_subtype": geometry_info.get("shape_subtype")
                    if visual_content
                    else None,
                    "stem_length": len(stem),
                    "has_geometry": visual_content is not None,
    ...
    if valid_scores:
        ...
        for r in results:
            if r.get("quality_score") is not None:
                ...
                print(
                    f"{emoji} Q{r['question_id']}: {r['test_case']:<50} Score: {score:.3f}"
                )
            else:
        ...
        for r in results:
            if r.get("quality_score") is not None:
                gt = r.get("geometry_type", "Unknown")
                subtype = r.get("shape_subtype", "Unknown")
                key = f"{gt} ({subtype})"
                if key not in geometry_type_scores:
                    geometry_type_scores[key] = []
                geometry_type_scores[key].append(r["quality_score"])

        for gt, scores in geometry_type_scores.items():
        ...
        print(f"  Phase 2 (Graphs):   Avg: {phase2_avg:.3f}")
        print(f"  Phase 3 (Geometry): Avg: {avg_score:.3f}")

        # Compare with best previous phase
        best_prev = max(phase1_avg, phase2_avg)
        diff = avg_score - best_prev
        if diff >= 0:
            print(
                f"  Difference from best: +{diff:.3f} (Phase 3 is {abs(diff):.1%} better)"
            )
        else:
            print(
                f"  Difference from best: {diff:.3f} (Phase 3 is {abs(diff):.1%} lower)"
            )
    ...
    evaluation_report = {
        ...
        "comparison_with_previous_phases": {
            "phase1_avg": 0.897,
            "phase2_avg": 0.900,
            "phase3_avg": avg_score if valid_scores else None,
            "difference_from_phase1": (avg_score - 0.897) if valid_scores else None,
            "difference_from_phase2": (avg_score - 0.900) if valid_scores else None,
        },
        "results": results,
    }
