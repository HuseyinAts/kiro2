...
class CommandBus:
    ...
    def register(self, command_type: type[Command], handler: CommandHandler):
        ...
        logger.info(
            f"Registered CommandHandler: {handler.__class__.__name__} for {command_type.__name__}"
        )
    ...
    async def execute(self, command: TCommand) -> Any:
        ...
        try:
            ...
            logger.debug(
                f"Command {command_type.__name__} executed successfully in {duration:.2f}ms"
            )
            return result
        except Exception as e:
            duration = (time.perf_counter() - start_time) * 1000
            logger.error(
                f"Command {command_type.__name__} failed after {duration:.2f}ms: {e}",
                exc_info=True,
            )
            raise
...
class QueryBus:
    ...
    def register(self, query_type: type[Query], handler: QueryHandler):
        ...
        logger.info(
            f"Registered QueryHandler: {handler.__class__.__name__} for {query_type.__name__}"
        )
    ...
    async def execute(self, query: TQuery) -> Any:
        ...
        try:
            ...
            duration = (time.perf_counter() - start_time) * 1000
            logger.debug(
                f"Query {query_type.__name__} executed successfully in {duration:.2f}ms"
            )
            return result
        except Exception as e:
            duration = (time.perf_counter() - start_time) * 1000
            logger.error(
                f"Query {query_type.__name__} failed after {duration:.2f}ms: {e}",
                exc_info=True,
            )
            raise
