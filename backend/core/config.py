...
class EmbeddingConfig:
    ...
    """
    MODEL_OPTIONS: dict[str, str] = {EmbeddingModelType.BERTURK: 'emrecan/bert-base-turkish-cased-mean-nli-stsb-tr', EmbeddingModelType.MULTILINGUAL: 'paraphrase-multilingual-mpnet-base-v2'}
    DEFAULT_MODEL: str = os.getenv('EMBEDDING_MODEL', EmbeddingModelType.BERTURK)
    MODEL_DIMENSIONS: dict[str, int] = {EmbeddingModelType.BERTURK: 768, EmbeddingModelType.MULTILINGUAL: 768}
...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        self.database_test_url = os.getenv('DATABASE_TEST_URL', 'sqlite+aiosqlite:///./test_turkiye_sinav.db')
        ...
        if not self.jwt_secret_key:
            ...
            if self.environment == 'production':
                raise ValueError('CRITICAL SECURITY ERROR: JWT_SECRET_KEY environment variable is required in production!')
        if self.environment == 'production':
            ...
            if unique_chars < 32:
                raise ValueError(f'CRITICAL: JWT key low entropy ({unique_chars}/32 chars)!')
        ...
        origins_str = os.getenv('ALLOWED_ORIGINS', 'http://localhost:3000,http://localhost:3001,http://localhost:5173')
        ...
        self.feature_flags = {'new_exam_ui': os.getenv('FF_NEW_EXAM_UI', 'false').lower() == 'true', 'bionic_reading': os.getenv('FF_BIONIC_READING', 'true').lower() == 'true', 'dark_mode': os.getenv('FF_DARK_MODE', 'true').lower() == 'true', 'ai_tutor': os.getenv('FF_AI_TUTOR', 'false').lower() == 'true', 'ai_question_generation': os.getenv('FF_AI_QUESTION_GEN', 'false').lower() == 'true', 'gemini_integration': os.getenv('FF_GEMINI', 'true').lower() == 'true', 'irt_adaptive': os.getenv('FF_IRT_ADAPTIVE', 'true').lower() == 'true', 'fsrs_spaced_repetition': os.getenv('FF_FSRS', 'true').lower() == 'true', 'zpd_optimization': os.getenv('FF_ZPD', 'true').lower() == 'true', 'turkish_nlp': os.getenv('FF_TURKISH_NLP', 'true').lower() == 'true', 'zemberek_integration': os.getenv('FF_ZEMBEREK', 'true').lower() == 'true', 'premium_analytics': os.getenv('FF_PREMIUM_ANALYTICS', 'false').lower() == 'true', 'video_solutions': os.getenv('FF_VIDEO_SOLUTIONS', 'false').lower() == 'true', 'beta_multi_agent': os.getenv('FF_BETA_MULTI_AGENT', 'false').lower() == 'true', 'beta_voice_input': os.getenv('FF_BETA_VOICE', 'false').lower() == 'true'}
        self._validate_settings()

    def _validate_settings(self):
        """Ayarları doğrula"""
        valid_db_prefixes = ('postgresql://', 'postgresql+asyncpg://', 'sqlite+aiosqlite://')
        ...
        if self.environment == 'production':
            if self.jwt_secret_key == 'your-secret-key-change-in-production':
                raise ValueError('CRITICAL SECURITY ERROR: Production JWT secret key is using default value! Set JWT_SECRET_KEY environment variable to a strong random key.')
            if len(self.jwt_secret_key) < 32:
                raise ValueError('CRITICAL SECURITY ERROR: JWT secret key must be at least 32 characters long!')
        if self.environment == 'production' and self.secret_key == 'your-secret-key-change-in-production':
            raise ValueError('CRITICAL SECURITY ERROR: Production SECRET_KEY is using default value!')
        if self.environment == 'production':
    ...
    def _validate_production_environment(self):
        ...
        if self.youtube_api_key:
            ...
            if 'test' in self.youtube_api_key.lower() or 'example' in self.youtube_api_key.lower():
