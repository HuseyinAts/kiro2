"""345 2025 AYT Matematik kitabinin eski hat source_book yazimini ASCII'ye cevirir

Revision ID: 0046_mat345ayt_kaynak_adi
Revises: 0045_mat345ayt_agac
Create Date: 2026-09-24

NEDEN
-----
`question_metadata.source_book` bu depoda bir etiket degil KIMLIKtir
(bkz. scripts/kitap/kaynak_sozlesmesi.py). Yeni ithal (mat345ayt_ithal.py)
ASCII adi kullanir:

    "345 2025 AYT Matematik Soru Bankasi"

DB'de bu kitaptan eski hattan gelen 11 satir var ve yazimi:

    "345 2025 Ayt Matematik Soru Bankas<U+0131>"   <- 'Ayt', dotless-i
...
sessizce delik birakir (0019 Aromat, 0030 Bilgi Sarmal, 0043 345 TYT ile ayni durum).
...
'345 2024 Ayt Matematik Soru Bankas<U+0131>' (10 satir, onceki baski) FARKLI
anahtara cozuluyor, ona dokunulmaz.
...
revision: str = "0046_mat345ayt_kaynak_adi"
down_revision: Union[str, None] = "0045_mat345ayt_agac"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

_log = logging.getLogger("alembic.runtime.migration")

GUNLUK = "mat345ayt_kaynak_adi_gunlugu_0046"

# Kaynak dosyasi ASCII kalsin diye Turkce harf chr() ile yazilir.
ESKI = "345 2025 Ayt Matematik Soru Bankas" + chr(0x0131)
YENI = "345 2025 AYT Matematik Soru Bankasi"
...
def upgrade() -> None:
    b = op.get_bind()
    if not _tablo_var(b):
        _log.info("[0046] question_metadata yok (taze DB?) -- atlandi")
        return
    ...
    if not hedef:
        _log.info("[0046] eski yazimla satir yok -- atlandi")
        return
    ...
    _log.info("[0046] source_book yazimi duzeltildi: %s satir", len(hedef))


def downgrade() -> None:
    b = op.get_bind()
    if not sa.inspect(b).has_table(GUNLUK):
        _log.info("[0046] %s yok -- downgrade atlandi", GUNLUK)
        return
    if not _tablo_var(b):
    ...
    _log.info("[0046] geri alindi: %s satir eski yazimina dondu", len(kayitlar))
    op.drop_table(GUNLUK)
