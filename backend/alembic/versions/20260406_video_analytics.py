...
def upgrade():
    op.create_table("video_completion_milestones",
        ...
        sa.Column("badge_id", sa.String, sa.ForeignKey("user_badges.id"), nullable=True),
        sa.UniqueConstraint("user_id", "video_id", "milestone_percentage", name="idx_user_video_milestone"),
    )
    ...
    op.create_table("video_notes",
        ...
        sa.Column("video_id", sa.String(100), nullable=False, index=True),
        sa.Column("video_source", sa.String(20), nullable=False),
        sa.Column("session_id", UUID(as_uuid=True), sa.ForeignKey("video_watch_sessions.id"), nullable=True),
    ...
    op.create_table("video_analytics_summary",
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.UniqueConstraint("user_id", "period_type", "period_start", name="idx_user_period"),
    )
