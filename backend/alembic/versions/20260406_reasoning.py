...
def upgrade():
    ...
    op.create_table("reasoning_steps",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("session_id", UUID(as_uuid=True), sa.ForeignKey("reasoning_sessions.id", ondelete="CASCADE"), nullable=False),
        sa.Column("step_number", sa.Integer, nullable=False),
        sa.Column("step_type", sa.String(20), default="inference"),
        sa.Column("description", sa.Text, nullable=False),
        sa.Column("reasoning", sa.Text, nullable=True),
        sa.Column("result", sa.Text, nullable=True),
        sa.Column("parent_step_id", UUID(as_uuid=True), sa.ForeignKey("reasoning_steps.id", ondelete="SET NULL"), nullable=True),
