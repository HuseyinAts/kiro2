...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('passed_gates', sa.Integer(), server_default='0',
                  comment='Number of passed gates'),
        sa.Column('failed_gates', sa.Integer(), server_default='0',
                  comment='Number of failed gates'),
        sa.Column('skipped_gates', sa.Integer(), server_default='0',
        ...
        sa.Column('parallel_execution_used', sa.Boolean(), server_default='false',
                  comment='Whether parallel execution was used'),
        sa.Column('fail_fast_mode', sa.Boolean(), server_default='false',
        ...
        sa.Column('override_approver', sa.String(255), nullable=True,
    ...
    op.create_table(
        ...
        # Request info
        sa.Column('gate_name', sa.String(100), nullable=False,
                  comment='Gate being overridden'),
        sa.Column('run_id', postgresql.UUID(as_uuid=True), nullable=True,
                  comment='Related pipeline run ID'),

        # Requestor
        sa.Column('requestor', sa.String(255), nullable=False,
    ...
    op.create_index(
        'idx_qg_override_gate',
        'quality_gates_override_audit',
        ['gate_name']
    )
    op.create_index(
        'idx_qg_override_requestor',
        'quality_gates_override_audit',
        ['requestor']
    )
    op.create_index(
        'idx_qg_override_status',
        'quality_gates_override_audit',
        ['status']
    )
    op.create_index(
        'idx_qg_override_created',
        'quality_gates_override_audit',
        ['created_at']
    )
    op.create_index(
