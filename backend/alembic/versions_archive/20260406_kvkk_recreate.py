...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_consents_user_id ON kvkk_consents(user_id)"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_consents_purpose ON kvkk_consents(purpose)"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_consents_status ON kvkk_consents(status)"
        )
    )
    ...
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_deletion_user_id ON kvkk_data_deletion_requests(user_id)"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_deletion_status ON kvkk_data_deletion_requests(status)"
        )
    )
    ...
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_audit_user_id ON kvkk_audit_logs(user_id)"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_audit_action ON kvkk_audit_logs(action)"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_kvkk_audit_created_at ON kvkk_audit_logs(created_at)"
        )
    )
