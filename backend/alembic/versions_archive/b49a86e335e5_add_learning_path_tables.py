...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_update_req_type", "curriculum_update_requests", ["update_type"]
    )
    ...
    op.create_index(
        "idx_fallback_subject_topic", "fallback_videos", ["subject", "topic"]
    )
    ...
    op.create_index(
        "idx_meb_subject_grade", "meb_curriculum_standards", ["subject", "grade_level"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["meb_standard_id"], ["meb_curriculum_standards.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["osym_standard_id"], ["osym_standards.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "idx_outcome_meb_standard", "learning_outcomes", ["meb_standard_id"]
    )
    ...
    op.create_index(
        "idx_student_exam_target", "learning_path_student_profiles", ["exam_target"]
    )
    op.create_index("idx_student_grade", "learning_path_student_profiles", ["grade"])
    op.create_index(
        "idx_student_last_activity",
        "learning_path_student_profiles",
        ["last_activity_at"],
    )
    op.execute(
        sa.text(
            "CREATE INDEX IF NOT EXISTS idx_student_learning_style ON learning_path_student_profiles (learning_style)"
        )
    )
    op.create_index(
        "idx_student_user_id", "learning_path_student_profiles", ["user_id"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["student_id"], ["learning_path_student_profiles.student_id"]
        ),
        sa.PrimaryKeyConstraint("path_id"),
    )
    op.create_index("idx_path_created_at", "learning_paths", ["created_at"])
    op.create_index(
        "idx_path_student_subject", "learning_paths", ["student_id", "subject"]
    )
    ...
    op.create_index(
        "idx_quiz_question_order", "quiz_questions", ["quiz_id", "order_number"]
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "score >= 0 AND score <= 100", name="check_quiz_score_range"
        ),
        sa.ForeignKeyConstraint(
            ["student_id"], ["learning_path_student_profiles.student_id"]
        ),
    ...
    op.create_index(
        "idx_quiz_student_quiz", "quiz_submissions", ["student_id", "quiz_id"]
    )
    ...
    op.create_index(
        "idx_progress_student_node", "topic_progress", ["student_id", "node_id"]
    )
