...
try:
    ...
    try:
        ...
        # NLTK eksik korpus icin LookupError firlatir -- indirilmesi gereken
        # tek durum bu. Ciplak `except` agi/izin hatalarini da yutuyordu.
...
class TurkishTextPreprocessor:
    ...
    def __init__(self):
        ...
        # Kivrik tirnaklar + en/em dash: string.punctuation'da OLMAYAN
        # tipografik karakterler. Kacis dizileri BILINCLI -- bu satirdaki
        # kivrik tirnaklar bir noktada duz ASCII tirnaga duzlestirilmis,
        # bastaki uclu `\"` bir uc-tirnak ACICISI olmus ve dosya
        # ayristirilamaz hale gelmisti (CodeQL 'syntax errors', 28 Agu 2026).
    ...
    def _load_turkish_stopwords(self) -> Set[str]:
        ...
        if NLTK_AVAILABLE:
            try:
                ...
                # Korpus indirilmemis: asagidaki manuel liste yeterli fallback.
                # Sessiz `pass` yerine iz birak -- hangi kaynagin kullanildigi
                # gorunur olsun.
                logging.warning(
                    "NLTK 'turkish' stopwords korpusu yok; manuel listeye dusuldu"
                )
