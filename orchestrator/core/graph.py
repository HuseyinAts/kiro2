...
from .routing import RoutingEngine, RoutingDecision, TaskType
from .quality_gates import QualityGatePipeline, GateAction
...
"""
    ...
    async def _implement_node(self, state: OrchestratorState) -> OrchestratorState:
        ...
        state["implementation_result"] = f"Implementation completed (iteration {state['iteration']})"
    ...
    async def _quality_check_node(self, state: OrchestratorState) -> OrchestratorState:
        ...
        state["gate_results"] = [
            {
                "gate": self.quality_pipeline.gates[i].config.name if i < len(self.quality_pipeline.gates) else "unknown",
                "success": out.success,
...
"""
    ...
    def _quality_check_router(self, state: OrchestratorState) -> Literal["review", "fix", "blocked"]:
    ...
    async def get_status(self, run_id: str) -> Optional[OrchestratorState]:
        """Çalışma durumunu al"""
