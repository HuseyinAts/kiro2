# Golden Flow Wave History (Waves 1-16)

Context-frozen archive split from `.claude/rules/golden-flows.md` (Session 156, 14 May 2026).
The main rule keeps the read-path table (GF1-GF8), the saturation declaration, and the
backlog summary. Detailed per-wave probe tables, root-cause writeups, and hit-rate
trailing-indicator analysis live here.

For active rules, current PASS/FAIL distribution, backlog status, and CI gate
definition see `.claude/rules/golden-flows.md`. This archive starts at the
Wave 1 write-path probes (the GF1-GF8 read-path table is preserved in the
main rule).
...
For the post-Wave-16 backlog (schema drift migrations, sync-service async
ports, Session 153+ "DONE" status), see the "Next development phase" section
