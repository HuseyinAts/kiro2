...
| 1.5+++ | 🎯 **Tier H q_index_in_page exact (Session 158 SON)** | **GIZLI KEŞIF**: `pipeline_metadata.ai_extras.q_index_in_page` field page-içi sıra = disk filename `qNN` DIREKT eşleşir. Tier C/D/E/F/G `q_no` (kitap test #) kullanıyordu, q_index_in_page (page-içi) kullanmıyordu. **+49,468 image_url, missing %10.13 → %2.51 ✓ HEDEF SAĞLANDI**. Script: `tier_h_qip_exact.py`, RESULT: `_pilots/20260515_TIER_H_BREAKTHROUGH_RESULT.md` |
