...
| 1.5++ | ✅ **Tier G kombineli derin recovery (Session 158)** | 8 hipotez sistematik test (H5 has_diagram=NULL gizli kategori +%1, H6 page offset reddedildi). Tier G: G1 (key+sim>=0.40) + G2 (page no-key sim>=0.55) + G3 (no q_no sim>=0.55). Scope: has_diagram=true OR (NULL+visual cue). +2,493 match. Missing %14.96 → **%10.13**. RESULT: `_pilots/20260515_DEEP_ROOT_CAUSE_RESULT.md` |
| 1.5+++ | ❌ **Tier H q_index_in_page — ROLLBACK (Session 158 audit)** | **BUG**: DB qip %92.9 sayfa 0-indexed, disk filename 1-indexed. Tier H v1 offset bug → 49,468 satırın ~%75'i yanlış image_url. ROLLBACK uygulandı (commit pending). v2 (offset-aware) pilot da %75 yanlış → q_index_in_page Gemini-assigned, deterministic değil. **Tier H konsepti iptal**. Pipeline-fix bound REVIZE: **%10 missing** (Tier C+D+E+F+G ile). RESULT: `_pilots/20260515_DB_COMPREHENSIVE_AUDIT_RESULT.md` |
